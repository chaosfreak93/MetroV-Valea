    ...
    public class Main : IScript
    {
        ...
        public static void Initialize() {
            ...
            var data = new Server_Peds {
                model = "s_m_m_pilot_01", posX = startJobShape.Position.X, posY = startJobShape.Position.Y, posZ = startJobShape.Position.Z - 1,
                rotation = -24f
            };
            ServerPeds.ServerPeds_.Add(data);
            var markerData = new Server_Markers {
                type = 33, posX = Constants.Positions.Minijob_Pilot_VehOutPos.X, posY = Constants.Positions.Minijob_Pilot_VehOutPos.Y,
                posZ = Constants.Positions.Minijob_Pilot_VehOutPos.Z + 0.25f, scaleX = 1, scaleY = 1, scaleZ = 1, red = 46, green = 133, blue = 232,
                alpha = 150, bobUpAndDown = true
            };
            ...
            MinijobSpots_.Add(new Minijob_Spots {
                id = 2,
                depositShape = Alt.CreateColShapeSphere(
                    new Position((float) 2011.5601806640625, (float) 4743.40966796875, (float) 41.199241638183594), 2.5f)
            }); //Grapeseed
            MinijobSpots_.Add(new Minijob_Spots {
            ...
            try {
                ...
                if (player.GetPlayerCurrentMinijobStep() == "DRIVE_BACK_TO_START" &&
                    ...
                    ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                        DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Eingehende Überweisung", "San Andreas Flights",
                        "Minijob Gehalt", $"+{rnd}$", "Online Banking");
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast den Minijob erfolgreich abgeschlossen. Dein Gehalt i.H.v. {rnd}$ wurde dir auf dein Hauptkonto überwiesen. Du hast {rndExp}EXP dazu bekommen.");
        ...
        private static void PlayerEnterVehicle(IVehicle vehicle, IPlayer player, byte seat) {
            try {
                ...
                if (player.GetPlayerCurrentMinijobStep() == "FirstStepInVehicle") {
                    ...
                    player.EmitLocked("Client:Minijob:CreateJobMarker", "Pilot: Checkpoint", 3, 514, 30, MinijobSpots_[0].depositShape.Position.X,
                        MinijobSpots_[0].depositShape.Position.Y, MinijobSpots_[0].depositShape.Position.Z, true);
                }
            }
            catch (Exception e) {
                Alt.Log($"{e}");
        ...
        private static void ColShapeEnterExitHandler(IColShape colShape, IEntity targetEntity, bool state) {
            try {
                ...
                if (colShape == MinijobSpots_[0].depositShape && state && client.IsInVehicle) {
                    ...
                    HUDHandler.SendNotification(client, 2, 15000,
                        "Am Checkpoint angekommen, warte 15 Sekunden - fliege anschließend zum nächsten Flughafen.");
                    client.EmitLocked("Client:Minijob:CreateJobMarker", "Grapeseed Airport", 3, 514, 30, MinijobSpots_[1].depositShape.Position.X,
                        MinijobSpots_[1].depositShape.Position.Y, MinijobSpots_[1].depositShape.Position.Z, true);
                    ...
                    HUDHandler.SendNotification(client, 2, 15000,
                        "Am Checkpoint angekommen, warte 15 Sekunden - fliege anschließend zum nächsten Flughafen.");
                    client.EmitLocked("Client:Minijob:CreateJobMarker", "Sandy Shores Airport", 3, 514, 30,
                        MinijobSpots_[2].depositShape.Position.X, MinijobSpots_[2].depositShape.Position.Y,
                        MinijobSpots_[2].depositShape.Position.Z, true);
                    ...
                    client.EmitLocked("Client:Minijob:RemoveJobMarkerWithFreeze", 15000);
                    HUDHandler.SendNotification(client, 2, 15000,
                        "Am Checkpoint angekommen, warte 15 Sekunden - fliege anschließend zurück zum International Airport und gebe dein Flugzeug ab.");
                    client.SetPlayerCurrentMinijobActionCount(0);
                    client.SetPlayerCurrentMinijobStep("DRIVE_BACK_TO_START");
                    client.EmitLocked("Client:Minijob:CreateJobMarker", "Pilot: Flugzeugabgabe", 3, 514, 30,
                        Constants.Positions.Minijob_Pilot_VehOutPos.X, Constants.Positions.Minijob_Pilot_VehOutPos.Y,
                        Constants.Positions.Minijob_Pilot_VehOutPos.Z, true);
                }
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            ...
            try {
                ...
                foreach (var veh in Alt.GetAllVehicles().ToList()) {
                    ...
                    if (veh.Position.IsInRange(Constants.Positions.Minijob_Pilot_VehOutPos, 8f)) {
                        HUDHandler.SendNotification(player, 3, 5000, "Der Hangar ist blockiert.");
                        return;
                    }
                }
                ...
                switch (level) {
                    ...
                    case 2:
                        if (CharactersMinijobs.GetCharacterMinijobEXP(charId, "Pilot") < 50) {
                            HUDHandler.SendNotification(player, 3, 5000,
                                $"Du hast nicht die nötigen EXP für diese Stufe (50 EXP - du hast {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Pilot")}EXP).");
                            return;
                        }
                    ...
                    case 3:
                        if (CharactersMinijobs.GetCharacterMinijobEXP(charId, "Pilot") < 100) {
                            HUDHandler.SendNotification(player, 3, 5000,
                                $"Du hast nicht die nötigen EXP für diese Stufe (100 EXP - du hast {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Pilot")}EXP).");
                            return;
                        }
                ...
                HUDHandler.SendNotification(player, 1, 2500,
                    "Du hast den Minijob begonnen. Wir haben dir ein Flugzeug im Hangar abgestellt, steige ein.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
