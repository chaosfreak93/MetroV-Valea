...
namespace Altv_Roleplay.Minijobs.Busfahrer
{
    public class Main : IScript
    {
        public static ClassicColshape startJobShape =
            (ClassicColshape) Alt.CreateColShapeSphere(Constants.Positions.Minijob_Busdriver_StartPos, 2f);
        ...
        public static void Initialize() {
            ...
            var data = new Server_Peds {
                model = "cs_tom", posX = startJobShape.Position.X, posY = startJobShape.Position.Y, posZ = startJobShape.Position.Z - 1,
                rotation = -106.16410064697266f
            };
            ServerPeds.ServerPeds_.Add(data);
            var markerData = new Server_Markers {
                type = 39, posX = Constants.Positions.Minijob_Busdriver_VehOutPos.X, posY = Constants.Positions.Minijob_Busdriver_VehOutPos.Y,
                posZ = Constants.Positions.Minijob_Busdriver_VehOutPos.Z + 1, alpha = 150, bobUpAndDown = true, scaleX = 1, scaleY = 1, scaleZ = 1,
                red = 46, green = 133, blue = 232
            };
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat) {
            try {
                ...
                var givenEXP = Model.GetRouteGivenEXP((int) player.GetPlayerCurrentMinijobRouteId());
                var givenMoney = Model.GetRouteGivenMoney((int) player.GetPlayerCurrentMinijobRouteId());
                ...
                if (!CharactersBank.HasCharacterBankMainKonto(charId)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Dein Gehalt i.H.v. {givenMoney}$ konnte nicht überwiesen werden da du kein Hauptkonto hast. Du hast aber {givenEXP}EXP erhalten (du hast nun: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP).");
                    return;
                }
                ...
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Eingehende Überweisung", "Los Santos Transit",
                    "Minijob Gehalt", $"+{givenMoney}$", "Online Banking");
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Du hast den Minijob erfolgreich abgeschlossen. Dein Gehalt i.H.v. {givenMoney}$ wurde dir auf dein Hauptkonto überwiesen. Du hast {givenEXP} erhalten (deine EXP: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")})");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        private static void ColshapeEnterExitHandler(IColShape colShape, IEntity targetEntity, bool state) {
            try {
                ...
                var charId = User.GetPlayerOnline(client);
                if (charId <= 0) return;
                ...
                if (client.GetPlayerCurrentMinijobStep() == "DRIVE_TO_NEXT_STATION" && state && client.IsInVehicle &&
                    ServerVehicles.GetVehicleType(client.Vehicle) == 2) {
                    ...
                    var maxSpots = Model.GetMinijobMaxRouteSpots((int) client.GetPlayerCurrentMinijobRouteId());
                    ...
                    if ((int) client.GetPlayerCurrentMinijobActionCount() < maxSpots) {
                        ...
                        client.EmitLocked("Client:Minijob:CreateJobMarker", "Minijob: Haltestelle", 3, 80, 30, newSpot.posX, newSpot.posY,
                            newSpot.posZ, false);
                        HUDHandler.SendNotification(client, 2, 10000,
                            "An Haltestelle angekommen, warte 10 Sekunden und fahre anschließend zur nächsten Haltestelle.");
                        ...
                        //zurueck zum Depot
                        HUDHandler.SendNotification(client, 2, 10000,
                            "An Haltestelle angekommen, warte 10 Sekunden und fahre den Bus anschließend zurück zum Depot und stelle ihn dort ab, wo du ihn bekommen hast.");
                        client.SetPlayerCurrentMinijobStep("DRIVE_BACK_TO_START");
                        client.EmitLocked("Client:Minijob:CreateJobMarker", "Minijob: Busabgabe", 3, 515, 30,
                            Constants.Positions.Minijob_Busdriver_VehOutPos.X, Constants.Positions.Minijob_Busdriver_VehOutPos.Y,
                            Constants.Positions.Minijob_Busdriver_VehOutPos.Z, false);
                    }
                }
            }
            catch (Exception e) {
        ...
        public void StartMiniJob(IPlayer player, int routeId) {
            try {
                ...
                if (CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer") < Model.GetRouteNeededEXP(routeId)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Du hast nicht die benötigen EXP für diese Linie ({Model.GetRouteNeededEXP(routeId)}EXP - du hast {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP).");
                    return;
                }
                ...
                foreach (var veh in Alt.GetAllVehicles().ToList()) {
                    ...
                    if (veh.Position.IsInRange(Constants.Positions.Minijob_Busdriver_VehOutPos, 8f)) {
                        HUDHandler.SendNotification(player, 3, 5000, "Der Ausparkpunkt ist blockiert.");
                        return;
                    }
                }
                ...
                HUDHandler.SendNotification(player, 1, 2500,
                    "Du hast den Minijob begonnen. Wir haben dir einen Bus am Tor ausgeparkt, steige ein.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
