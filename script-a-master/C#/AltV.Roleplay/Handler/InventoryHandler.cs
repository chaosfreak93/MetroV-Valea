    ...
    internal class InventoryHandler : IScript
    {
        ...
        public static void RequestInventoryItems(IPlayer player) {
            try {
                ...
                var invArray = CharactersInventory.GetCharacterInventory(User.GetPlayerOnline(player));
                player.EmitLocked("Client:Inventory:AddInventoryItems", invArray,
                    Characters.GetCharacterBackpackSize(Characters.GetCharacterBackpack(User.GetPlayerOnline(player))), 0);
        ...
        public void switchItemToDifferentInv(ClassicPlayer player, string itemname, int itemAmount, string fromContainer, string toContainer) {
            try {
                ...
                var normalName = ServerItems.ReturnNormalItemName(itemname);
                ...
                if (toContainer == "inventory") {
                    if (invWeight + itemWeight > 15f) {
                        HUDHandler.SendNotification(player, 3, 5000, "Soviel Platz hast du im Inventar nicht.");
                        return;
                    }
                    ...
                    if (backpackWeight + itemWeight > Characters.GetCharacterBackpackSize(Characters.GetCharacterBackpack(charId))) {
                        HUDHandler.SendNotification(player, 3, 5000, "Soviel Platz hast du in deinen Taschen / deinem Rucksack nicht.");
                        return;
                    }
                }
                ...
                if (CharactersInventory.GetCharacterItemAmount(charId, itemname, fromContainer) < itemAmount) {
                    HUDHandler.SendNotification(player, 4, 5000,
                        "Die angegebene Item-Anzahl ist größer als die Anzahl der Items die du mit dir trägst.");
                    return;
                }

                if (itemname == "Rucksack" || itemname == "Tasche" || normalName == "Ausweis" || normalName == "Bargeld" ||
                    normalName == "Smartphone" || normalName == "EC Karte" || normalName == "Fahrzeugschluessel") {
                    HUDHandler.SendNotification(player, 3, 5000, "Diesen Gegenstand kannst du nicht in deinen Rucksack / deine Tache legen.");
                    return;
                }
                ...
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Der Gegenstand {itemname} ({itemAmount}x) wurde erfolgreich vom ({fromContainer}) in ({toContainer}) verschoben.");
        ...
        public void UseItem(ClassicPlayer player, string itemname, int itemAmount, string fromContainer) {
            try {
                ...
                if (ServerItems.IsItemUseable(ServerItems.ReturnNormalItemName(itemname)) == false) {
                    HUDHandler.SendNotification(player, 4, 5000, $"Dieser Gegenstand ist nicht benutzbar ({itemname})!");
                    return;
                }
                ...
                if (CharactersInventory.GetCharacterItemAmount(charId, itemname, fromContainer) < itemAmount) {
                    HUDHandler.SendNotification(player, 4, 5000, $"Die angegeben zu nutzende Anzahl ist nicht vorhanden ({itemname})!");
                    return;
                }

                if (itemname.Contains("EC Karte")) {
                    var SplittedItemName = itemname.Split(' ');
                    ECData = itemname.Replace("EC Karte ", "");
                    itemname = "EC Karte";
                } else if (itemname.Contains("Fahrzeugschluessel")) {
                    var SplittedItemName = itemname.Split(' ');
                    CarKeyData = itemname.Replace("Fahrzeugschluessel ", "");
                    itemname = "Autoschluessel";
                }

                if (ServerItems.IsItemDesire(itemname)) {
                    CharactersInventory.RemoveCharacterItemAmount(charId, itemname, itemAmount, fromContainer);
                    Characters.SetCharacterHunger(charId,
                        Characters.GetCharacterHunger(charId) + ServerItems.GetItemDesireFood(itemname) * itemAmount);
                    Characters.SetCharacterThirst(charId,
                        Characters.GetCharacterThirst(charId) + ServerItems.GetItemDesireDrink(itemname) * itemAmount);
                    player.EmitLocked("Client:HUD:UpdateDesire", Characters.GetCharacterHunger(charId),
                        Characters.GetCharacterThirst(charId)); //HUD updaten
                ...
                if (itemname == "Rucksack" || itemname == "Tasche") {
                    if (fromContainer == "backpack") {
                        HUDHandler.SendNotification(player, 3, 5000, "Kleidungen & Taschen können nicht aus dem Rucksack aus benutzt werden.");
                        return;
                    }
                    ...
                    if (Characters.GetCharacterBackpack(charId) == 31) {
                        if (itemname == "Rucksack") {
                            ...
                            HUDHandler.SendNotification(player, 3, 5000,
                                "Du hast bereits eine Tasche angelegt, lege diese vorher ab um deinen Rucksack anzulegen.");
                        }
                        ...
                        if (itemname == "Tasche") {
                            if (CharactersInventory.GetCharacterBackpackItemCount(charId) == 0) {
                                ...
                                HUDHandler.SendNotification(player, 4, 5000,
                                    "Du hast zuviele Sachen in deiner Tasche, du kannst deine Tasche nicht ablegen.");
                            }
                        } else {
                            HUDHandler.SendNotification(player, 3, 5000,
                                "Du hast bereits einen Rucksack angelegt, lege diesen vorher ab um deine Tasche anzulegen.");
                        }
                    ...
                    if (atmPos == null || player.IsInVehicle) {
                        HUDHandler.SendNotification(player, 3, 5000, "Du bist an keinem ATM oder sitzt in einem Auto.");
                        return;
                    }

                    var usingAccountNumber = Convert.ToInt32(ECData);
                    ...
                    if (CharactersBank.GetBankAccountLockStatus(usingAccountNumber)) {
                        ...
                        HUDHandler.SendNotification(player, 3, 5000, "Ihre EC Karte wurde einzogen da diese gesperrt ist.");
                        return;
                    }

                    player.EmitLocked("Client:ATM:BankATMcreateCEF", CharactersBank.GetBankAccountPIN(usingAccountNumber), usingAccountNumber,
                        atmPos.zoneName);
                } else if (ServerItems.GetItemType(itemname) == "weapon") {
                    if (itemname.Contains("Munitionsbox")) {
                        var wName = itemname.Replace(" Munitionsbox", "");
                        CharactersInventory.RemoveCharacterItemAmount(charId, itemname, itemAmount, fromContainer);
                    ...
                    var house = ServerHouses.ServerHouses_.FirstOrDefault(x =>
                        x.ownerId > 0 && x.isLocked && ((ClassicColshape) x.entranceShape).IsInRange(player));
                    ...
                    if (player.Health == 200) {
                        HUDHandler.SendNotification(player, 4, 2000, "Dir geht es schon sehr gut!");
                        return;
                    }
                    ...
                    if (player.Health + 15 >= 200) {
                        Characters.SetCharacterHealth(charId, 200);
                        player.Health = 200;
                        return;
                    }

                    Characters.SetCharacterHealth(charId, Characters.GetCharacterHealth(charId) + 15);
                    var health = player.Health + 15;
                    player.SetHealthAsync(ushort.Parse(health.ToString()));
                } else if (itemname == "Tablette") {
                    if (!CharactersInventory.ExistCharacterItem(charId, "Wasser", "inventory") &&
                        !CharactersInventory.ExistCharacterItem(charId, "Wasser", "backpack")) {
                        HUDHandler.SendNotification(player, 4, 2000, "Du brauchst Wasser um die Tablette zu nehmen!");
                        return;
                    }

                    if (CharactersInventory.ExistCharacterItem(charId, "Wasser", "inventory"))
                        CharactersInventory.RemoveCharacterItemAmount(charId, "Wasser", 1, "inventory");
                    else if (CharactersInventory.ExistCharacterItem(charId, "Wasser", "backpack"))
                        CharactersInventory.RemoveCharacterItemAmount(charId, "Wasser", 1, "backpack");
                    CharactersInventory.RemoveCharacterItemAmount(charId, "Tablette", 1, fromContainer);

                    if (player.Health + 5 >= 200) {
                        Characters.SetCharacterHealth(charId, 200);
                        player.Health = 200;
                        return;
                    }

                    Characters.SetCharacterHealth(charId, Characters.GetCharacterHealth(charId) + 5);
                    var health = player.Health + 5;
                    player.SetHealthAsync(ushort.Parse(health.ToString()));
                    ...
                    ServerVehicles.SetVehicleFuel(player.Vehicle, ServerVehicles.GetVehicleFuel(player.Vehicle) + 5.0f);
                    ...
                    if (player.Armor >= 60) {
                        HUDHandler.SendNotification(player, 3, 2000, "Weiter kannst du dich nicht selbst heilen.");
                        return;
                    }
                    ...
                    if (Characters.IsCharacterPhoneEquipped(charId)) {
                        ...
                        player.EmitLocked("Client:Smartphone:equipPhone", true, Characters.GetCharacterPhonenumber(charId),
                            Characters.IsCharacterPhoneFlyModeEnabled(charId), Characters.GetCharacterPhoneWallpaper(charId));
                    ...
                    SmartphoneHandler.RequestLSPDIntranet(player);
        ...
        public void DropItem(ClassicPlayer player, string itemname, int itemAmount, string fromContainer) {
            try {
                ...
                var normalItemName = ServerItems.ReturnNormalItemName(itemname);

                if (ServerItems.IsItemDroppable(itemname) == false) {
                    HUDHandler.SendNotification(player, 4, 5000, $"Diesen Gegenstand kannst du nicht wegwerfen ({itemname}).");
                    return;
                }
                ...
                if (itemname == "Smartphone") {
                    if (Characters.IsCharacterPhoneEquipped(charId)) {
                        HUDHandler.SendNotification(player, 3, 2500, "Du musst dein Handy erst ausschalten / ablegen.");
                        return;
                    }
                    ...
                    if (Characters.GetCharacterBackpack(charId) == 31) {
                        if (CharactersInventory.GetCharacterItemAmount(charId, "Rucksack", "inventory") == itemAmount) {
                            HUDHandler.SendNotification(player, 3, 5000,
                                "Du musst deinen Rucksack erst ablegen, bevor du diesen wegwerfen kannst.");
                            return;
                        }
                    ...
                    if ((string) Characters.GetCharacterWeapon(player, "PrimaryWeapon") == normalItemName ||
                        (string) Characters.GetCharacterWeapon(player, "SecondaryWeapon") == normalItemName ||
                        (string) Characters.GetCharacterWeapon(player, "SecondaryWeapon2") == normalItemName ||
                        (string) Characters.GetCharacterWeapon(player, "FistWeapon") == normalItemName)
                        if (CharactersInventory.GetCharacterItemAmount(charId, normalItemName, fromContainer) == itemAmount) {
                            HUDHandler.SendNotification(player, 3, 5000, "Du musst zuerst deine Waffe ablegen.");
                            return;
                        }
                }
        ...
        public void GiveItem(ClassicPlayer player, string itemname, int itemAmount, string fromContainer, int targetPlayerId) {
            try {
                ...
                if (!ServerItems.IsItemGiveable(itemname)) {
                    HUDHandler.SendNotification(player, 4, 5000, $"Diesen Gegenstand kannst du nicht weggeben ({itemname}).");
                    return;
                }
                ...
                if (CharactersInventory.GetCharacterItemAmount(charId, itemname, fromContainer) < itemAmount) {
                    HUDHandler.SendNotification(player, 4, 5000, $"Die angegebene Anzahl ist nicht vorhanden ({itemname}).");
                    return;
                }

                if (CharactersInventory.IsItemActive(player, itemname)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Ausgerüstete Gegenstände können nicht abgegeben werden.");
                    return;
                }
                ...
                if (!player.Position.IsInRange(targetPlayer.Position, 5f)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Die Person ist zu weit entfernt.");
                    return;
                }

                if (invWeight + itemWeight > 15f && backpackWeight + itemWeight >
                    Characters.GetCharacterBackpackSize(Characters.GetCharacterBackpack(targetPlayerId))) {
                    HUDHandler.SendNotification(player, 3, 5000, "Der Spieler hat nicht genug Platz in seinen Taschen.");
                    return;
                }
                ...
                if (Characters.GetCharacterBackpack(targetPlayerId) != -2 && backpackWeight + itemWeight <=
                    Characters.GetCharacterBackpackSize(Characters.GetCharacterBackpack(targetPlayerId))) {
                    HUDHandler.SendNotification(targetPlayer, 1, 5000, $"Eine Person hat dir den Gegenstand {itemname} ({itemAmount}x) gegeben.");
                    HUDHandler.SendNotification(player, 2, 5000, $"Du hast einem Spieler den Gegenstand {itemname} ({itemAmount}x) gegeben.");
        ...
        public void PlayerSearchTakeItem(ClassicPlayer player, int givenTargetCharId, string itemName, string itemLocation, int itemAmount) {
            try {
                ...
                if (!player.Position.IsInRange(targetPlayer.Position, 3f)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Fehler: Du bist zuweit vom Spieler entfernt.");
                    return;
                }

                if (!targetPlayer.HasPlayerHandcuffs() && !targetPlayer.HasPlayerRopeCuffs() && !targetPlayer.HasPlayerFootcuffs()) {
                    HUDHandler.SendNotification(player, 3, 5000, "Der Spieler ist nicht gefesselt.");
                    return;
                }

                if (!ServerItems.IsItemDroppable(itemName) || !ServerItems.IsItemGiveable(itemName)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Fehler: Diesen Gegenstand kannst du nicht entfernen.");
                    return;
                }

                if (!CharactersInventory.ExistCharacterItem(targetCharId, itemName, itemLocation)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Dieser Gegenstand existiert nicht mehr.");
                    return;
                }

                if (CharactersInventory.IsItemActive(targetPlayer, itemName)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Ausgerüstete Gegenstände können nicht entwendet werden.");
                    return;
                }

                if (CharactersInventory.GetCharacterItemAmount(targetCharId, itemName, itemLocation) < itemAmount) {
                    HUDHandler.SendNotification(player, 3, 5000, "Fehler: Soviele Gegenstände hat der Spieler davon nicht.");
                    return;
                }
                ...
                if (invWeight + itemWeight <= 15f || itemName == "Bargeld" || itemWeight == 0f) {
                    ...
                    HUDHandler.SendNotification(targetPlayer, 2, 5000,
                        $"Dir wurde der Gegenstand {itemName} ({itemAmount}x) aus dem Inventar entwendet.");
                ...
                if (Characters.GetCharacterBackpack(charId) != -2 && backpackWeight + itemWeight <=
                    ...
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast der Person {itemName} ({itemAmount}x) entwendet. (Lagerort: Rucksack/Tasche).");
                    HUDHandler.SendNotification(targetPlayer, 2, 5000,
                        $"Dir wurde der Gegenstand {itemName} ({itemAmount}x) aus dem Rucksack entwendet.");
        ...
        internal static void InventoryAnimation(IPlayer player, string Animation, int duration) {
            ...
            if (Animation == "eat")
                player.EmitLocked("Client:Inventory:PlayAnimation", "mp_player_inteat@burger", "mp_player_int_eat_burger", 3500, 49, false);
            else if (Animation == "drink") player.EmitLocked("Client:Inventory:PlayAnimation", "mp_player_intdrink", "intro", 3500, 49, false);
            else if (Animation == "drop") player.EmitLocked("Client:Inventory:PlayAnimation", "anim@narcotics@trash", "drop_front", 500, 1, false);
            else if (Animation == "give")
                player.EmitLocked("Client:Inventory:PlayAnimation", "anim@narcotics@trash", "drop_front", 500, 1,
            ...
            else if (Animation == "handcuffs") player.EmitLocked("Client:Inventory:PlayAnimation", "mp_arresting", "sprint", duration, 33, false);
            ...
            else if (Animation == "weste")
                player.EmitLocked("Client:Inventory:PlayAnimation", "anim@heists@narcotics@funding@gang_idle", "gang_chatting_idle01", 3000, 49,
                    false);
            else if (Animation == "Kokain")
                player.EmitLocked("Client:Inventory:PlayAnimation", "anim@heists@narcotics@funding@gang_idle", "gang_chatting_idle01", 2000, 49,
                    false);
            else if (Animation == "verband")
                player.EmitLocked("Client:Inventory:PlayAnimation", "anim@heists@narcotics@funding@gang_idle", "gang_chatting_idle01", 3000, 49,
                    false);
            else if (Animation == "Mechanic")
                player.EmitLocked("Client:Inventory:PlayAnimation", "mini@repair", "fixing_a_ped", duration, 1, false);
        }
