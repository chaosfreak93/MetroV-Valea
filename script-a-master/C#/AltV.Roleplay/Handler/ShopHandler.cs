...
{
    internal class ShopHandler : IScript
    {
        #region Shops
        ...
        public void buyShopItem(IPlayer player, int shopId, int amount, string itemname) {
            ...
            if (!player.Position.IsInRange(ServerShops.GetShopPosition(shopId), 3f)) {
                HUDHandler.SendNotification(player, 3, 5000, "Du bist zu weit vom Shop entfernt.");
                return;
            }
            ...
            if (ServerShops.GetShopNeededLicense(shopId) != "None" &&
                !Characters.HasCharacterPermission(charId, ServerShops.GetShopNeededLicense(shopId))) {
            ...
            var itemPrice = ServerShopsItems.GetShopItemPrice(shopId, itemname) * amount;
            var shopFaction = ServerShops.GetShopFaction(shopId);

            if (ServerShopsItems.GetShopItemAmount(shopId, itemname) < amount) {
                HUDHandler.SendNotification(player, 3, 5000, "Soviele Gegenstände hat der Shop nicht auf Lager.");
                return;
            }
            ...
            if (invWeight + itemWeight <= 15f) {
                if (shopFaction > 0 && shopFaction != 0) {
                    if (!ServerFactions.IsCharacterInAnyFaction(charId)) {
                        HUDHandler.SendNotification(player, 3, 2500, "Du hast hier keinen Zugriff drauf [CODE1-2].");
                        return;
                    }

                    if (ServerFactions.GetCharacterFactionId(charId) != shopFaction) {
                        HUDHandler.SendNotification(player, 3, 2500,
                            $"Du hast hier keinen Zugriff drauf (Gefordert: {shopFaction} - Deine: {ServerFactions.GetCharacterFactionId(charId)}.");
                        return;
                    }

                    if (ServerFactions.GetFactionBankMoney(shopFaction) < itemPrice) {
                        HUDHandler.SendNotification(player, 3, 2500, "Die Frakton hat nicht genügend Geld auf dem Fraktionskonto.");
                        return;
                    }

                    ServerFactions.SetFactionBankMoney(shopFaction, ServerFactions.GetFactionBankMoney(shopFaction) - itemPrice);
                    LoggingService.NewFactionLog(shopFaction, charId, 0, "shop",
                        $"{Characters.GetCharacterName(charId)} hat {itemname} ({amount}x) für {itemPrice}$ erworben.");
                } else {
                    if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory") ||
                        CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < itemPrice) {
            ...
            if (Characters.GetCharacterBackpack(charId) != -2 &&
                ...
                if (shopFaction > 0 && shopFaction != 0) {
                    if (!ServerFactions.IsCharacterInAnyFaction(charId)) {
                        HUDHandler.SendNotification(player, 3, 2500, "Du hast hier keinen Zugriff drauf [CODE1].");
                        return;
                    }
                ...
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Du hast {itemname} ({amount}x) für {itemPrice} gekauft (Lagerort: Rucksack / Tasche).");
                stopwatch.Stop();
                if (stopwatch.Elapsed.Milliseconds > 30) Alt.Log($"{charId} - buyShopItem benötigte {stopwatch.Elapsed.Milliseconds}ms");
            }
        }
            ...
            try {
                ...
                if (shopPos.faction > 0 && shopPos.faction != 0) {
                    ...
                    if (ServerFactions.GetCharacterFactionId(charId) != shopPos.faction) {
                        HUDHandler.SendNotification(player, 3, 2500,
                            $"Kein Zugriff [{shopPos.faction} - {ServerFactions.GetCharacterFactionId(charId)}]");
                        return;
                    }
                }
                ...
                if (shopPos.isOnlySelling == false) {
                    Global.mGlobal.VirtualAPI.TriggerClientEventSafe(player, "Client:Shop:shopCEFCreateCEF",
                        ServerShopsItems.GetShopShopItems(shopPos.shopId), shopPos.shopId, shopPos.isOnlySelling);
                    stopwatch.Stop();
                    if (stopwatch.Elapsed.Milliseconds > 30) Alt.Log($"{charId} - openShop benötigte {stopwatch.Elapsed.Milliseconds}ms");
                    return;
                }

                if (shopPos.isOnlySelling) {
                    Global.mGlobal.VirtualAPI.TriggerClientEventSafe(player, "Client:Shop:shopCEFCreateCEF",
                        ServerShopsItems.GetShopSellItems(charId, shopPos.shopId), shopPos.shopId, shopPos.isOnlySelling);
                    stopwatch.Stop();
                    if (stopwatch.Elapsed.Milliseconds > 30) Alt.Log($"{charId} - openShop benötigte {stopwatch.Elapsed.Milliseconds}ms");
                    return;
                }
        ...
        public async void robShop(ClassicPlayer player, int shopId) {
            try {
                ...
                if (player.IsUnconscious) {
                    HUDHandler.SendNotification(player, 3, 5000, "Du bist bewusstlos, der Raub wurde abgebrochen.");
                    return;
                }

                var amount = new Random().Next(10000, 15000);
        ...
        public void sellShopItem(IPlayer player, int shopId, int amount, string itemname) {
            ...
            if (!CharactersInventory.ExistCharacterItem(charId, itemname, "inventory") &&
                !CharactersInventory.ExistCharacterItem(charId, itemname, "backpack")) {
                HUDHandler.SendNotification(player, 3, 5000, "Diesen Gegenstand besitzt du nicht.");
                return;
            }

            var itemSellPrice = ServerShopsItems.GetShopItemPrice(shopId, itemname); //Verkaufpreis pro Item
            var invItemAmount = CharactersInventory.GetCharacterItemAmount(charId, itemname, "inventory"); //Anzahl an Items im Inventar
            var backpackItemAmount = CharactersInventory.GetCharacterItemAmount(charId, itemname, "backpack"); //Anzahl an Items im Rucksack

            if (invItemAmount + backpackItemAmount < amount) {
                HUDHandler.SendNotification(player, 3, 5000, "Soviele Gegenstände hast du nicht zum Verkauf dabei.");
                return;
            }
        ...
        public void BuyVehicle(IPlayer player, int shopid, string hash) {
            try {
                ...
                if (!PlaceFree) {
                    HUDHandler.SendNotification(player, 3, 5000, "Der Ausladepunkt ist belegt.");
                    return;
                }
                ...
                if (ServerVehicles.ExistServerVehiclePlate($"NL{rnd}")) {
                    BuyVehicle(player, shopid, hash);
                    return;
                }

                if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory") ||
                    CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < Price) {
                    HUDHandler.SendNotification(player, 4, 5000, $"Du hast nicht genügend Bargeld dabei ({Price}$).");
                    return;
                }
        ...
        public static void openClothesShop(ClassicPlayer player, int id) {
            try {
                ...
                if (!player.HasData("clothesMenuOpen")) {
                    player.SetData("clothesMenuOpen", true);
                } else {
        ...
        public void buyClothesShopItem(ClassicPlayer player, int clothId, bool isProp) {
            try {
                if (player == null || !player.Exists || player.CharacterId <= 0 ||
                    !ServerClothes.ExistClothes(clothId, Convert.ToInt32(Characters.GetCharacterGender(player.CharacterId)))) return;

                Characters.SwitchCharacterClothes(player, clothId, isProp);

                if (CharactersClothes.ExistCharacterClothes(player.CharacterId, clothId)) {
                    HUDHandler.SendNotification(player, 2, 1500, "Du hast das Kleidungsstück angezogen.");
                } else {
                    var price = ServerClothesShops.GetClothesPrice(player, clothId, isProp);
        ...
        public void ClothesshopSetPerfectTorso(ClassicPlayer player, int BestTorsoDrawable, int BestTorsoTexture) {
            try {
                var clothId = ServerClothes.GetClothesId(3, BestTorsoDrawable, BestTorsoTexture,
                    Convert.ToInt32(Characters.GetCharacterGender(player.CharacterId)));
                if (player == null || !player.Exists || player.CharacterId <= 0 ||
                    !ServerClothes.ExistClothes(clothId, Convert.ToInt32(Characters.GetCharacterGender(player.CharacterId)))) return;
        ...
        public void ClientEvent_buyTattoo(ClassicPlayer player, int shopId, int tattooId) {
            try {
                if (player == null || !player.Exists || player.CharacterId <= 0 || shopId <= 0 || tattooId <= 0 ||
                    !ServerTattoos.ExistTattoo(tattooId) || CharactersTattoos.ExistAccountTattoo(player.CharacterId, tattooId) ||
                    !ServerTattooShops.ExistTattooShop(shopId)) return;

                var price = ServerTattoos.GetTattooPrice(tattooId);
        ...
        public void ClientEvent_deleteTattoo(ClassicPlayer player, int tattooId) {
            try {
                if (player == null || !player.Exists || player.CharacterId <= 0 || tattooId <= 0 ||
                    !CharactersTattoos.ExistAccountTattoo(player.CharacterId, tattooId)) return;

                CharactersTattoos.RemoveAccountTattoo(player.CharacterId, tattooId);
                HUDHandler.SendNotification(player, 2, 1500,
                    $"Du hast das Tattoo '{ServerTattoos.GetTattooName(tattooId)}' erfolgreich entfernen lassen.");
                player.updateTattoos();
            }
            catch (Exception e) {
                Console.WriteLine($"{e}");
