    ...
    internal class Functions : IScript
    {
        ...
        public async void RepairVehicle(IPlayer player, IVehicle vehicle) {
            try {
                ...
                if (ServerFactions.GetCharacterFactionId(((ClassicPlayer) player).CharacterId) == 4) {
                    InventoryHandler.InventoryAnimation(player, "Mechanic", 6000);
                    await Task.Delay(6000);
                } else {
                    InventoryHandler.InventoryAnimation(player, "Mechanic", 60000);
                    await Task.Delay(60000);
                }

                ServerVehicles.SetVehicleEngineHealthy(vehicle, true);
                vehicle.SetVehicleTrunkState(false);
                vehicle.SetVehicleEngineHoodState(false);
                AltAsync.EmitAllClients("Client:Utilities:repairVehicle", vehicle);
        ...
        public void TowVehicle(IPlayer player, IVehicle vehicle) {
            try {
                ...
                if (charId <= 0 || player.HasPlayerRopeCuffs() || player.HasPlayerHandcuffs() || player.IsPlayerUnconscious() ||
                    !ServerFactions.IsCharacterInAnyFaction(charId) ||
                    !vehicle.Position.IsInRange(Constants.Positions.AutoClubLosSantos_StoreVehPosition, 5f) || vehId <= 0) return;
                if (ServerFactions.GetCharacterFactionId(charId) != 4) return;

                var vehClass = ServerAllVehicles.GetVehicleClass(vehicle.Model);
                ...
                LoggingService.NewFactionLog(4, charId, vehId, "towVehicle",
                    $"{Characters.GetCharacterName(charId)} hat das Fahrzeug mit der ID {vehId} abgeschleppt.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
