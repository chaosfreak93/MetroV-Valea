...
alt.onServer('Client:Tablet:createCEF', ()=>{
    ...
    let animInterval = alt.setInterval(()=>{
        ...
        game.taskPlayAnim(alt.Player.local.scriptID, "cellphone@", "cellphone_cellphone_intro", 1, -1, -1, 50, 0, false, false, false);
...
alt.onServer("Client:Tablet:SetLifeinvaderAppContent", (array)=>{
    if (tabletBrowser != null) {
        tabletBrowser.emit("CEF:Tablet:SetLifeinvaderAppEventEntrys", array);
    }
});
...
let openTabletCEF = function() {
    if (tabletBrowser == null && alt.Player.local.getSyncedMeta("IsCefOpen") == false && alt.Player.local.getSyncedMeta("PLAYER_SPAWNED") == true) {
        ...
        tabletBrowser.on("Client:Tablet:LifeinvaderAppNewEntry", LifeinvaderAppNewEntry);
...
function LifeinvaderAppNewEntry(title, callNumber, information) {
    if (!canInteract) return;
    lastInteract = Date.now();
    alt.emitServer("Server:Tablet:LifeinvaderAppNewEntry", title, callNumber, information);
}
...
export function closeTabletCEF() {
    if (tabletBrowser != null) {
        ...
        tabletBrowser.off("Client:Tablet:LifeinvaderAppNewEntry", LifeinvaderAppNewEntry);
