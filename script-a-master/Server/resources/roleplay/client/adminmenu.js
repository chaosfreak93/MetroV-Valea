...
alt.onServer("Client:AdminMenu:GetWaypointInfo", () => {
    ...
    if (game.doesBlipExist(waypoint)) {
        ...
        game.startPlayerTeleport(alt.Player.local.scriptID, coords.x, coords.y, coords.z + 1, alt.Player.local.rot.toDegrees(), true, true, false);
        alt.setTimeout(() => {
...
export default class NoClip {
    ...
    static keyHandler() {
        ...
        if (game.isControlPressed(0, NoClip.KEYS.SHIFT)) speed = speed * 5;

        if (game.isControlPressed(0, NoClip.KEYS.FORWARD)) currentPos = addSpeedToVector(currentPos, dirForward, speed);
        if (game.isControlPressed(0, NoClip.KEYS.BACKWARD)) currentPos = addSpeedToVector(currentPos, dirForward, -speed);
        if (game.isControlPressed(0, NoClip.KEYS.LEFT)) currentPos = addSpeedToVector(currentPos, dirRight, -speed, true);
        if (game.isControlPressed(0, NoClip.KEYS.RIGHT)) currentPos = addSpeedToVector(currentPos, dirRight, speed, true);
        let zModifier = 0;
        if (game.isControlPressed(0, NoClip.KEYS.UP)) zModifier += speed;
        if (game.isControlPressed(0, NoClip.KEYS.DOWN)) zModifier -= speed;

        if (!isVectorEqual(new alt.Vector3(currentPos.x, currentPos.y, currentPos.z + zModifier), alt.Player.local.pos)) game.setEntityCoordsNoOffset(alt.Player.local.scriptID, currentPos.x, currentPos.y, currentPos.z + zModifier, 0, 0, 0);
    }
}