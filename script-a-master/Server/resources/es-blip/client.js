    ...
    if (currEntityData) {
        ...
        if (data != undefined) {
            if (entityType === BLIPTYPE) {
                staticBlipStreamer.addBlip(+entityId, position, data.name, data.sprite, data.color, data.scale, data.shortRange);
            } else if (entityType === BLIPTYPE2) {
                dynamicBlipStreamer.addBlip(+entityId, position, data.name, data.sprite, data.color, data.scale, data.shortRange);
            }
        }
    } else {
        if (entityType === BLIPTYPE) {
