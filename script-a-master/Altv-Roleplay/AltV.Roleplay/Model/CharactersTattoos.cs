...
{
    internal class CharactersTattoos
    {
        public static List<Characters_Tattoos> CharactersTattoos_ = new();
        ...
        public static string GetAccountOwnTattoos(int charId) {
            try {
                return JsonSerializer.Serialize(CharactersTattoos_.ToList().Where(x => x.charId == charId).Select(x => new {
                    x.tattooId,
