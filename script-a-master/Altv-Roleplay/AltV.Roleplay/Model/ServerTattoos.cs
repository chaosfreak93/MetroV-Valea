...
{
    internal class ServerTattoos
    {
        public static List<Server_Tattoos> ServerTattoos_ = new();
        ...
        public static void GetAllTattoos(ClassicPlayer player) {
            try {
                ...
                var itemCount = tattooItems.Count;
                var iterations = Math.Floor((decimal) (itemCount / 30));
                var rest = itemCount % 30;

                for (var i = 0; i < iterations; i++) {
                    var skip = i * 30;
                    player.Emit("Client:TattooShop:sendItemsToClient",
                        JsonSerializer.Serialize(tattooItems.Skip(skip).Take(30).ToList()));
                }

                if (rest != 0)
                    player.Emit("Client:TattooShop:sendItemsToClient",
                        JsonSerializer.Serialize(tattooItems.Skip((int) iterations * 30).ToList()));
            }
            catch (Exception e) {
                Console.WriteLine($"{e}");
            }
