...
{
    internal class ServerShopsItems
    {
        public static List<Server_Shops_Items> ServerShopsItems_ = new();
        ...
        public static string GetShopSellItems(int charId, int shopId) {
            ...
            var items = ServerShopsItems_.Where(x =>
                x.shopId == shopId && (CharactersInventory.ExistCharacterItem(charId, x.itemName, "inventory") ||
                                       CharactersInventory.ExistCharacterItem(charId, x.itemName, "backpack"))).Select(x => new {
                itemname = x.itemName,
                itemprice = x.itemPrice,
                itempic = ServerItems.ReturnItemPicSRC(x.itemName),
                itemmaxamount = CharactersInventory.GetCharacterItemAmount(charId, x.itemName, "inventory") +
                                CharactersInventory.GetCharacterItemAmount(charId, x.itemName, "backpack")
            }).ToList();
        ...
        public static void RemoveShopItemAmount(int shopId, string itemName, int itemAmount) {
            ...
            var prevAmount = shopItem.itemAmount;
            ...
            using (var db = new gtaContext()) {
                if (shopItem.itemAmount > 0) {
                    ...
                    RemoveShopItem(shopId, itemName);
                }
            }
        }
