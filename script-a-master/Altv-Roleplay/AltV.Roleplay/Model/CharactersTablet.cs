...
{
    internal class CharactersTablet
    {
        public static List<Characters_Tablet_Apps> CharactersTabletApps_ = new();
        public static List<Characters_Tablet_Tutorial> CharactersTabletTutorialData_ = new();
        public static List<Server_Tablet_Apps> ServerTabletAppsData_ = new();
        public static List<Server_Tablet_Events> ServerTabletEventsData_ = new();
        public static List<Server_Tablet_Advertisements> ServerTabletAdvertisementsData_ = new();
        public static List<Server_Tablet_Notes> ServerTabletNotesData_ = new();

        public static void CreateCharacterTabletAppEntry(int charId, bool weather, bool news, bool banking, bool lifeinvader, bool vehicles,
            bool events, bool company, bool notices) {
            if (charId == 0) return;
        ...
        public static void CreateServerTabletEvent(int charId, string title, string callNumber, string eventDate, string eventTime, string location,
            string eventType, string info) {
            try {
                if (charId == 0) return;
        ...
        public static void CreateServerTabletAdvertisement(int charId, int factionId, string title, string callNumber, string info) {
            try {
                if (charId == 0) return;

                var advertisementData = new Server_Tablet_Advertisements {
                    charId = charId,
                    factionId = factionId,
                    title = title,
                    ownerName = Characters.GetCharacterName(charId),
                ...
                ServerTabletAdvertisementsData_.Add(advertisementData);

                using (var db = new gtaContext()) {
                    db.Server_Tablet_Advertisements.Add(advertisementData);
                    db.SaveChanges();
                }
            }
            catch (Exception e) {
        ...
        public static void SetCharacterTutorialEntryState(int charId, string entry, bool isCompleted) {
            try {
                ...
                switch (entry) {
                    case "openTablet":
                        tutData.openTablet = isCompleted;
                        break;
                    case "openInventory":
                        tutData.openInventory = isCompleted;
                        break;
                    case "createBankAccount":
                        tutData.createBankAccount = isCompleted;
                        break;
                    case "buyVehicle":
                        tutData.buyVehicle = isCompleted;
                        break;
                    case "useGarage":
                        tutData.useGarage = isCompleted;
                        break;
                    case "acceptJob":
                        tutData.acceptJob = isCompleted;
                        break;
                }
        ...
        public static void ChangeCharacterTabletAppInstallState(int charId, string appName, bool isInstalled) {
            try {
                ...
                if (tabletData != null) {
                    switch (appName) {
                        case "weather":
                            tabletData.weather = isInstalled;
                            break;
                        case "news":
                            tabletData.news = isInstalled;
                            break;
                        case "banking":
                            tabletData.banking = isInstalled;
                            break;
                        case "lifeinvader":
                            tabletData.lifeinvader = isInstalled;
                            break;
                        case "vehicles":
                            tabletData.vehicles = isInstalled;
                            break;
                        case "events":
                            tabletData.events = isInstalled;
                            break;
                        case "company":
                            tabletData.company = isInstalled;
                            break;
                        case "notices":
                            tabletData.notices = isInstalled;
                            break;
                    }
        ...
        public static void SetCharacterTabletApps(int charId, bool weather, bool news, bool banking, bool lifeinvader, bool vehicles, bool events,
            bool company, bool notices) {
            try {
                if (charId == 0) return;
        ...
        public static string GetCharacterTabletApps(int charId) {
            ...
            var items = CharactersTabletApps_.Where(x => x.charId == charId).Select(x => new {
                ...
                x.notices,
                factionmanager = ServerFactions.IsCharacterInAnyFaction(charId) && ServerFactions.GetCharacterFactionRank(charId) >
        ...
        public static string GetServerTabletAdvertisements() {
            var items = ServerTabletAdvertisementsData_.Where(x => DateTime.Now.Subtract(Convert.ToDateTime(x.created)).TotalHours < 168).Select(
                x => new {
                    x.id,
                    x.title,
                    x.factionId,
