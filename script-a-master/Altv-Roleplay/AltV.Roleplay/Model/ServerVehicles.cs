...
{
    internal class ServerVehicles
    {
        public static List<Server_Vehicles> ServerVehicles_ = new();
        ...
        public static List<Server_Vehicles> ServerVehiclesLocked_
        {
            get
            {
                lock (ServerVehicles_) {
                    return ServerVehicles_.ToList();
                }
            }
            set => ServerVehicles_ = value;
        }

        public static void CreateServerVehicle(int id, int charid, uint hash, int vehType, int factionid, float fuel, float km, bool engineState,
            bool isEngineHealthy, bool lockState, bool isInGarage, int garageId, Position position, Rotation rotation, string plate,
            DateTime lastUsage, DateTime buyDate) {
            ...
            var veh = Alt.CreateVehicle(hash, position, rotation);

            if (position == new Position(0, 0, 0)) {
                SetVehicleInGarage(veh, true, 1);
                return;
            }
        ...
        public static void AddVehicleTrunkItem(int vehId, string itemName, int itemAmount, bool inGlovebox) {
            ...
            try {
                ...
                var hasItem = ServerVehicleTrunkItems_.FirstOrDefault(i =>
                    i.vehId == vehId && i.itemName == itemName && i.isInGlovebox == inGlovebox);
                ...
                if (hasItem != null) {
                    ...
                    using (var db = new gtaContext()) {
                        var dbitem = db.Server_Vehicle_Items.FirstOrDefault(i =>
                            i.vehId == vehId && i.itemName == itemName && i.isInGlovebox == inGlovebox);
        ...
        public static bool ExistVehicleTrunkItem(int vehId, string itemName, bool isInGlovebox) {
            try {
                ...
                var trunkData =
                    ServerVehicleTrunkItems_.FirstOrDefault(x => x.vehId == vehId && x.itemName == itemName && x.isInGlovebox == isInGlovebox);
        ...
        public static string GetVehicleTrunkItems(int vehId, bool isInGlovebox) {
            var items = ServerVehicleTrunkItems_.ToList().Where(x => x != null && x.vehId == vehId && x.isInGlovebox == isInGlovebox).Select(x =>
                new {
                    x.id,
                    x.vehId,
                    x.itemName,
        ...
        public static void SetVehicleEngineState(IVehicle veh, bool state) {
            ...
            if (vehs != null) {
                AltAsync.Do(() => { veh.EngineOn = state; });
        ...
        public static void SetVehicleFuel(IVehicle veh, float fuel) {
            try {
                ...
                if (vehs != null) {
                    ...
                    } else if (vehs.fuel > GetVehicleFuelLimitOnHash(veh.Model)) {
                        vehs.fuel = GetVehicleFuelLimitOnHash(veh.Model);
                    }
                }
            }
            catch (Exception e) {
        ...
        public static void SetVehicleInGarage(IVehicle veh, bool state, int garageId) {
            try {
                ...
                if (state) {
                    ...
                    dbVehicle.engineState = false;
                    dbVehicle.lockState = true;
                    veh.Remove();
                }
        ...
        public static float GetVehicleVehicleTrunkWeight(int vehId, bool searchOnlyGlovebox) {
            try {
                ...
                if (searchOnlyGlovebox) {
                    var gItem = ServerVehicleTrunkItems_.ToList().Where(i => i.vehId == vehId && i.isInGlovebox);

                    foreach (var i in gItem) {
            ...
            try {
                ...
                if (vehs != null) {
                    ...
                    if (veh.LockState == VehicleLockState.Locked)
                        vehs.lockState = true;
                    else if (veh.LockState == VehicleLockState.Unlocked)
                        vehs.lockState = false;
                ...
                if (mod != null) {
                    ...
                    switch (modType) {
                        case 0:
                            ...
                            break;
                        case 1:
                            mod.front_bumper = modId;
                            veh.SetMod((byte) modType, (byte) modId);
                            break;
                        case 2:
                            mod.rear_bumper = modId;
                            veh.SetMod((byte) modType, (byte) modId);
                            break;
                        case 3:
                            mod.side_skirt = modId;
                            veh.SetMod((byte) modType, (byte) modId);
                            break;
                        case 4:
                            mod.exhaust = modId;
