...
{
    internal class ServerHotels
    {
        public static List<Server_Hotels> ServerHotels_ = new();
        public static List<Server_Hotels_Apartments> ServerHotelsApartments_ = new();
        public static List<Server_Hotels_Storage> ServerHotelsStorage_ = new();
        ...
        internal static void RequestHotelApartmentItems(IPlayer player, int hotelId) {
            try {
                ...
                for (var i = 0; i < iterations; i++) {
                    ...
                    player.EmitLocked("Client:Hotel:setApartmentItems", JsonSerializer.Serialize(items.Skip(skip).Take(20).ToList()));
                }

                if (player == null || !player.Exists) return;

                if (rest != 0)
                    player.EmitLocked("Client:Hotel:setApartmentItems", JsonSerializer.Serialize(items.Skip((int) iterations * 20).ToList()));
