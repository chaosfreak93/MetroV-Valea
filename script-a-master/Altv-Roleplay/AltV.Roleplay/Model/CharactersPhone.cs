...
{
    internal class CharactersPhone : IScript
    {
        public static List<CharactersPhoneChats> CharactersPhoneChats_ = new();
        public static List<CharactersPhoneChatMessages> CharactersPhoneChatMessages_ = new();
        public static List<CharactersPhoneContacts> CharactersPhoneContacts_ = new();
        ...
        public static bool ExistChatByNumbers(int phoneNumber, int anotherNumber) {
            try {
                return CharactersPhoneChats_.ToList().Exists(x =>
                    x.phoneNumber == phoneNumber && x.anotherNumber == anotherNumber ||
                    x.phoneNumber == anotherNumber && x.anotherNumber == phoneNumber);
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public static void RequestChatJSON(ClassicPlayer player, int playerNumber) {
            try {
                ...
                var chats = CharactersPhoneChats_.ToList().Where(x => x.phoneNumber == playerNumber || x.anotherNumber == playerNumber).Select(x =>
                    new {
                        x.chatId,
                        from = x.phoneNumber,
                        to = x.anotherNumber,
                ...
                var itemCount = chats.Count;
                var iterations = Math.Floor((decimal) (itemCount / 5));
                var rest = itemCount % 5;

                for (var i = 0; i < iterations; i++) {
                    var skip = i * 5;
                    player.EmitLocked("Client:Smartphone:addChatJSON", JsonSerializer.Serialize(chats.Skip(skip).Take(5).ToList()));
                }

                if (rest != 0)
                    player.EmitLocked("Client:Smartphone:addChatJSON", JsonSerializer.Serialize(chats.Skip((int) iterations * 5).ToList()));
