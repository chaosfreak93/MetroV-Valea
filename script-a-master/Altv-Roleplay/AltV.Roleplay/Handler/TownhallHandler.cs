...
namespace Altv_Roleplay.Handler
{
    internal class TownhallHandler : IScript
    {
        ...
        internal static void tryCreateIdentityCardApplyForm(IPlayer player) {
            ...
            var gender = Characters.GetCharacterGender(charId);
        ...
        public void SelectJobcenterJob(IPlayer player, string jobName) {
            try {
                ...
                if (jobName == "None") {
                    HUDHandler.SendNotification(player, 2, 5000, $"Du hast deinen Job als {Characters.GetCharacterJob(charId)} gekündigt.");
                    Characters.SetCharacterLastJobPaycheck(charId, DateTime.Now);
                    Characters.SetCharacterJob(charId, "None");
                    return;
                }
                ...
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Du hast den Vertrag für den Beruf '{jobName}' unterschrieben. Dein Gehalt liegt bei {ServerJobs.GetJobPaycheck(jobName)}$. Du musst täglich {ServerJobs.GetJobNeededHours(jobName)} Stunden anwesend sein.");
        ...
        internal static void openHouseSelector(IPlayer player) {
            try {
                ...
                var info = ServerHouses.GetAllCharacterHouses(charId);
