    ...
    internal class VehicleHandler : IScript
    {
        ...
        public void VehicleTrunkStorageItem(ClassicPlayer player, int vehId, int charId, string itemName, int itemAmount, string fromContainer,
            ...
            try {
                ...
                stopwatch.Start();
                if (player == null || !player.Exists || vehId <= 0 || charId <= 0 || itemName == "" || itemAmount <= 0 || fromContainer == "none" ||
                    fromContainer == "") return;
                ...
                if (!player.Position.IsInRange(targetVehicle.Position, 5f)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du bist zu weit entfernt.");
                    return;
                }

                if (type == "trunk") {
                    if (player.IsInVehicle) {
                        HUDHandler.SendNotification(player, 3, 5000, "Wie willst du von Innen an den Kofferraum kommen?");
                        return;
                    }

                    if (!targetVehicle.GetVehicleTrunkState()) {
                        HUDHandler.SendNotification(player, 4, 5000, "Fehler: Der Kofferraum ist nicht geöffnet.");
                        return;
                    }
                } else if (type == "glovebox") {
                    if (!player.IsInVehicle) {
                        HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du bist in keinem Fahrzeug.");
                        return;
                    }
                }
                ...
                if (curVehWeight + itemWeight > maxVehWeight) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Fehler: Soviel passt hier nicht rein (Aktuell: {curVehWeight} |  Maximum: {maxVehWeight}).");
                    return;
                }
                ...
                if (type == "trunk") {
                    ...
                    HUDHandler.SendNotification(player, 2, 2500,
                        $"Du hast den Gegenstand '{itemName} ({itemAmount}x)' in das Handschuhfach gelegt.");
                    stopwatch.Stop();
                    Alt.Log($"{charId} - VehicleTrunkStorageItem benötigte {stopwatch.Elapsed.Milliseconds}ms");
                }
            }
            ...
            try {
                ...
                if (type == "trunk") {
                    ...
                    if (!ServerVehicles.ExistVehicleTrunkItem(vehId, itemName, false)) {
                        HUDHandler.SendNotification(player, 4, 5000, "Fehler: Der Gegenstand existiert hier nicht.");
                        return;
                    }

                    if (ServerVehicles.GetVehicleTrunkItemAmount(vehId, itemName, false) < itemAmount) {
                        HUDHandler.SendNotification(player, 4, 5000, "Fehler: Soviele Gegenstände sind nicht im Fahrzeug.");
                        return;
                    }
                } else if (type == "glovebox") {
                    if (!player.IsInVehicle) {
                ...
                if (invWeight + itemWeight <= 15f) {
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast {itemName} ({itemAmount}x) aus dem Fahrzeug genommen (Lagerort: Inventar).");
                ...
                if (Characters.GetCharacterBackpack(charId) != -2 && backpackWeight + itemWeight <=
                    ...
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast {itemName} ({itemAmount}x) aus dem Fahrzeug genommen (Lagerort: Rucksack / Tasche).");
        ...
        internal static void OpenLicensingCEF(IPlayer player) {
            try {
                ...
                if (!player.Position.IsInRange(Constants.Positions.VehicleLicensing_Position, 3f)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du hast dich zu weit entfernt.");
                    return;
                }
                ...
                if (vehicleList.Count <= 0) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        "Keines deiner Fahrzeuge steht hinter dem Rathaus (an der roten Fahrzeugmarkierung).");
                    return;
                }

                player.EmitLocked("Client:VehicleLicensing:openCEF", JsonSerializer.Serialize(vehicleList));
                Alt.Log($"{JsonSerializer.Serialize(vehicleList)}");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void LicensingAction(IPlayer player, string action, int vehId, string vehPlate, string newPlate) {
            try {
                ...
                if (ServerVehicles.GetVehicleOwner(veh) != charId) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Dieses Fahrzeug gehört nicht dir.");
                    return;
                }

                if (!veh.Position.IsInRange(Constants.Positions.VehicleLicensing_VehPosition, 10f)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Das Fahrzeug ist nicht am Zulassungspunkt (hinterm Rathaus).");
                    return;
                }

                if (!ServerVehicles.GetVehicleLockState(veh)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Das Fahrzeug muss zugeschlossen sein.");
                    return;
                }
                ...
                if (action == "anmelden") {
                    ...
                    if (ServerVehicles.ExistServerVehiclePlate(newPlate)) {
                        HUDHandler.SendNotification(player, 3, 5000, "Fehler: Dieses Nummernschild ist bereits vorhanden.");
                        return;
                    }

                    var stringIsValid = Regex.IsMatch(newPlate, @"[a-zA-Z0-9-]$");
                    ...
                    if (!validPlate) {
                        HUDHandler.SendNotification(player, 4, 5000, "Fehler: Das Wunschnummernschild enthält unzulässige Zeichen.");
                        return;
                    }

                    if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory")) {
                        HUDHandler.SendNotification(player, 3, 5000, "Fehler: Du hast kein Bargeld dabei (250$).");
                        return;
                    }

                    if (CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < 250) {
                        HUDHandler.SendNotification(player, 3, 5000, "Fehler: Du hast nicht genügend Bargeld dabei (250$).");
                        return;
                    }
                    ...
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast das Kennzeichen von dem Fahrzeug '{ServerVehicles.GetVehicleNameOnHash(veh.Model)}' auf {newPlate} geändert (Gebühr: 250$).");
                } else if (action == "abmelden") {
                    var rnd = new Random().Next(100000, 999999);

                    if (ServerVehicles.ExistServerVehiclePlate($"NL{rnd}")) {
                        LicensingAction(player, "abmelden", vehId, vehPlate, newPlate);
                        return;
                    }
                    ...
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast das Fahrzeug '{ServerVehicles.GetVehicleNameOnHash(veh.Model)}' mit dem Kennzeichen '{vehPlate}' abgemeldet.");
                }
            }
            catch (Exception e) {
                Alt.Log($"{e}");
