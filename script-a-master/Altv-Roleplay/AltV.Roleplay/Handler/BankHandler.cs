    ...
    internal class BankHandler : IScript
    {
        ...
        public void CreateNewBankAccount(IPlayer player, string zoneName) {
            ...
            if (CharactersBank.GetCharacterBankAccountCount(player) >= 2) {
                HUDHandler.SendNotification(player, 4, 5000, "Du kannst nur zwei Bankkonten gleichzeitig haben.");
                return;
            }

            var rndAccNumber = new Random().Next(1000000, 999999999);
            ...
            if (CharactersBank.ExistBankAccountNumber(rndAccNumber)) {
                HUDHandler.SendNotification(player, 3, 5000, "Es ist ein Fehler aufgetreten. Bitte versuchen Sie es erneut.");
                return;
            }

            CharactersBank.CreateBankAccount(charid, rndAccNumber, rndPin, zoneName);
            HUDHandler.SendNotification(player, 2, 5000,
                $"Sie haben erfolgreich ein Konto erstellt. Ihre Kontonummer lautet ({rndAccNumber}) - ihr PIN: ({rndPin}).");
        ...
        public void BankAccountAction(IPlayer player, string action, string accountNumberStr) {
            ...
            var accountNumber = int.Parse(accountNumberStr);
            var charid = User.GetPlayerOnline(player);
            ...
            if (action == "generatepin") {
                ...
                if (CharactersBank.GetBankAccountLockStatus(accountNumber))
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Sie haben Ihr Konto mit der Kontonummer ({accountNumber}) erfolgreich gesperrt.");
                else
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Sie haben Ihr Konto mit der Kontonummre ({accountNumber}) erfolgreich entsperrt.");
                ...
                if (accountNumber == CharactersBank.GetCharacterBankMainKonto(charid)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Dieses Konto ist bereits dein Hauptkonto.");
                    return;
                }
                ...
                if (CharactersBank.GetBankAccountLockStatus(accountNumber)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        "Ihr Konto ist gesperrt, entsperren Sie Ihr Konto bevor Sie eine neue Karte erhalten können.");
                    return;
                }
                ...
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Sie haben erfolgreich eine Kartenkopie für das Konto mit der Kontonummer ({accountNumber}) erhalten.");
            }
        }
        ...
        public void requestATMBankData(ClassicPlayer player, int accountNumber) {
            ...
            player.EmitLocked("Client:ATM:BankATMSetRequestedData", CharactersBank.GetBankAccountMoney(accountNumber),
        ...
        public void WithdrawATMMoney(IPlayer player, int accountNumber, int amount, string zoneName) {
            ...
            if (CharactersBank.GetBankAccountLockStatus(accountNumber)) {
                ...
                if (CharactersInventory.ExistCharacterItem(charid, "EC Karte " + accountNumber, "inventory"))
            ...
            if (CharactersBank.GetBankAccountMoney(accountNumber) < amount) {
                HUDHandler.SendNotification(player, 3, 5000, "Ihr Konto ist für diese Summe nicht ausreichend gedeckt.");
                return;
            }

            var dateTime = DateTime.Now;
            CharactersBank.SetBankAccountMoney(accountNumber,
                CharactersBank.GetBankAccountMoney(accountNumber) - amount); //Geld vom Konto abziehen
            CharactersInventory.AddCharacterItem(charid, "Bargeld", amount, "inventory"); //Spieler Geld geben
            ServerBankPapers.CreateNewBankPaper(accountNumber, dateTime.ToString("dd.MM.yyyy"), dateTime.ToString("HH.mm"), "Auszahlung", "None",
                "None", $"-{amount}$", zoneName);
        ...
        public void TryATMPin(IPlayer player, string action, int accountNumber) {
            ...
            if (action == "reset") {
                ...
                if (CharactersBank.GetBankAccountPinTrys(accountNumber) >= 3) {
                    ...
                    HUDHandler.SendNotification(player, 3, 5000,
                        "Sie haben die Geheimzahl zu oft falsch eingegeben, Ihre Karte wurde gesperrt und eingezogen.");
                    CharactersBank.ChangeBankAccountLockStatus(accountNumber);
                    CharactersInventory.RemoveCharacterItemAmount(charid, "EC Karte " + accountNumber, 1, "inventory");
                } else {
        ...
        public void DepositATMMoney(IPlayer player, int accountNumber, int amount, string zoneName) {
            ...
            if (!CharactersInventory.ExistCharacterItem(charid, "Bargeld", "inventory") ||
                CharactersInventory.GetCharacterItemAmount(charid, "Bargeld", "inventory") < amount) {
                HUDHandler.SendNotification(player, 3, 5000, $"Du hast nicht genug Bargeld in deinem Inventar dabei ({amount}$).");
                return;
            }

            var dateTime = DateTime.Now;
            CharactersBank.SetBankAccountMoney(accountNumber,
                CharactersBank.GetBankAccountMoney(accountNumber) + amount); //Geld aufs Konto packen
            CharactersInventory.RemoveCharacterItemAmount(charid, "Bargeld", amount, "inventory"); //Spieler Geld entfernen
            ServerBankPapers.CreateNewBankPaper(accountNumber, dateTime.ToString("dd.MM.yyyy"), dateTime.ToString("HH.mm"), "Einzahlung", "None",
                "None", $"+{amount}$", zoneName);
        ...
        public void TransferATMMoney(IPlayer player, int accountNumber, int targetNumber, int amount, string zoneName) {
            ...
            CharactersBank.SetBankAccountMoney(accountNumber,
                CharactersBank.GetBankAccountMoney(accountNumber) - amount); //Geld vom Konto abziehen
            CharactersBank.SetBankAccountMoney(targetNumber,
                CharactersBank.GetBankAccountMoney(targetNumber) + amount); //Geld aufs Zielkonto addieren
            ...
            ServerBankPapers.CreateNewBankPaper(accountNumber, Date, Time, "Ausgehende Überweisung", $"{targetNumber}", "None", $"-{amount}$",
                zoneName);
            ServerBankPapers.CreateNewBankPaper(targetNumber, Date, Time, "Eingehende Überweisung", $"{accountNumber}", "None", $"+{amount}$",
                zoneName);
            HUDHandler.SendNotification(player, 2, 5000,
            ...
            try {
                ...
                if (type == "faction") {
                    if (!ServerFactions.IsCharacterInAnyFaction(charid)) {
                        HUDHandler.SendNotification(player, 4, 5000, "Du bist in keiner Fraktion und hast keine Berechtigung dazu.");
                        return;
                    }

                    if (factionId != ServerFactions.GetCharacterFactionId(charid)) {
                        HUDHandler.SendNotification(player, 4, 5000, "Ein unerwarteter Fehler ist aufgetreten. [FACTIONBANK-001]");
                        return;
                    }

                    if (ServerFactions.GetCharacterFactionRank(charid) != ServerFactions.GetFactionMaxRankCount(factionId) &&
                        ServerFactions.GetCharacterFactionRank(charid) != ServerFactions.GetFactionMaxRankCount(factionId) - 1) {
                        HUDHandler.SendNotification(player, 3, 5000, "Du hast nicht den benötigten Rang um auf das Fraktionskonto zuzugreifen.");
                        return;
                    }
                } else if (type == "company") {
                    if (!ServerCompanys.IsCharacterInAnyServerCompany(charid)) {
                        HUDHandler.SendNotification(player, 4, 5000, "Du bist in keinem Unternehmen und hast keine Berechtigung dazu.");
                        return;
                    }

                    if (factionId != ServerCompanys.GetCharacterServerCompanyId(charid)) {
                        HUDHandler.SendNotification(player, 4, 5000, "Ein unerwarteter Fehler ist aufgetreten. [FACTIONBANK-0001]");
                        return;
                    }

                    if (ServerCompanys.GetCharacterServerCompanyRank(charid) < 1) {
                        HUDHandler.SendNotification(player, 3, 5000, "Du hast nicht den benötigten Rang um auf das Unternehmenskonto zuzugreifen.");
                        return;
                    }
                }

                if (!CharactersInventory.ExistCharacterItem(charid, "Bargeld", "inventory") ||
                    CharactersInventory.GetCharacterItemAmount(charid, "Bargeld", "inventory") < moneyAmount) {
                    HUDHandler.SendNotification(player, 3, 5000, "Du hast nicht genügend Bargeld zum Einzahlen dabei.");
                    return;
                }
                ...
                if (type == "faction") {
                    ...
                    LoggingService.NewFactionLog(factionId, charid, 0, "bank",
                        $"{Characters.GetCharacterName(charid)} ({charid}) hat {moneyAmount}$ auf das Fraktionskonto eingezahlt.");
                } else if (type == "company") {
                    ...
                    LoggingService.NewCompanyLog(factionId, charid, 0, "bank",
                        $"{Characters.GetCharacterName(charid)} ({charid}) hat {moneyAmount}$ auf das Unternehmenskonto eingezahlt.");
                }
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            ...
            try {
                ...
                if (type == "faction") {
                    ...
                    if (ServerFactions.GetFactionBankMoney(factionId) < moneyAmount) {
                        HUDHandler.SendNotification(player, 3, 5000, "Soviel Geld ist auf dem Fraktionskonto nicht vorhanden.");
                        return;
                    }
                    ...
                    LoggingService.NewFactionLog(factionId, charid, 0, "bank",
                        $"{Characters.GetCharacterName(charid)} ({charid}) hat {moneyAmount}$ vom Fraktionskonto abgebucht.");
                } else if (type == "company") {
                    if (!ServerCompanys.IsCharacterInAnyServerCompany(charid)) {
                        HUDHandler.SendNotification(player, 4, 5000, "Du bist in keinem Unternehmen und hast keine Berechtigung dazu.");
                        return;
                    ...
                    if (ServerCompanys.GetServerCompanyMoney(factionId) < moneyAmount) {
                        HUDHandler.SendNotification(player, 3, 5000, "Soviel Geld ist auf dem Unternehmenskonto nicht vorhanden.");
                        return;
                    }
                    ...
                    LoggingService.NewCompanyLog(factionId, charid, 0, "bank",
                        $"{Characters.GetCharacterName(charid)} ({charid}) hat {moneyAmount} vom Unternehmenskonto abgebucht.");
                }
            }
            catch (Exception e) {
                Alt.Log($"{e}");
