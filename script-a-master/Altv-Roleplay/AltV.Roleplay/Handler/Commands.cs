...
namespace Altv_Roleplay.Handler
{
    public class Commands : IScript
    {
        ...
        public void GiveItemCMD(IPlayer player, int itemAmount) {
            ...
            var charId = player.GetCharacterMetaId();
            if (charId <= 0) return;
        ...
        public void heyCMD(IPlayer player, string model) {
            ...
            var veh = Alt.CreateVehicle(model, player.Position, player.Rotation);
            veh.EngineOn = true;
        ...
        public void PosCMD(IPlayer player) {
            ...
            player.SendChatMessage($"X: {player.Position.X}, Y: {player.Position.Y}, Z: {player.Position.Z - 0.83}");
        }
        ...
        public static void CMD_parkVehicleById(IPlayer player, int vehId) {
            try {
                ...
                var currentGarageId = ServerVehicles.GetVehicleGarageId(vehicle);
        ...
        public void GiveItemCMD(IPlayer player, string itemName, int itemAmount) {
            ...
            if (!ServerItems.ExistItem(ServerItems.ReturnNormalItemName(itemName))) {
                HUDHandler.SendNotification(player, 4, 5000, $"Itemname nicht gefunden: {itemName}");
                return;
            }
...
*/
        ...
        public void GotoCMD(IPlayer player, int targetId) {
            ...
            try {
                ...
                var targetCharName = Characters.GetCharacterName(targetId);
                ...
                if (!Characters.ExistCharacterName(targetCharName)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Warnung: Der angegebene Charaktername wurde nicht gefunden ({targetCharName} - ID: {targetId}).");
                    return;
                }
                ...
                HUDHandler.SendNotification(targetPlayer, 1, 5000,
                    $"{Characters.GetCharacterName((int) player.GetCharacterMetaId())} hat sich zu dir teleportiert.");
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Du hast dich zu dem Spieler {Characters.GetCharacterName((int) targetPlayer.GetCharacterMetaId())} teleportiert.");
        ...
        public void GetHereCMD(IPlayer player, int targetId) {
            ...
            try {
                ...
                HUDHandler.SendNotification(targetPlayer, 1, 5000,
                    $"{Characters.GetCharacterName((int) player.GetCharacterMetaId())} hat dich zu Ihm teleportiert.");
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Du hast den Spieler {Characters.GetCharacterName((int) targetPlayer.GetCharacterMetaId())} zu dir teleportiert.");
        ...
        public void CMD_ResetHwId(IPlayer player, int accountId) {
            try {
                ...
                if (player.GetCharacterMetaId() <= 0 || player.AdminLevel() <= 0) {
                    HUDHandler.SendNotification(player, 4, 2500, "Keine Rechte.");
                    return;
                }

                if (!User.ExistPlayerById(accountId)) {
                    HUDHandler.SendNotification(player, 3, 2500, "Der Spieler existiert nicht.");
                    return;
                }
