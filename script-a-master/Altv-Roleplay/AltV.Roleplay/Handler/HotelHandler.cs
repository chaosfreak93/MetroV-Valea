...
{
    internal class HotelHandler : IScript
    {
            ...
            try {
                ...
                if (ServerHotels.HasCharacterAnApartment(charId)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Du besitzt bereits ein Hotelzimmer in dem Hotel '{ServerHotels.GetCharacterRentedHotelName(charId)}'.");
                    return;
                }

                if (ServerHotels.GetApartmentOwner(hotelId, apartmentId) > 0) {
                    HUDHandler.SendNotification(player, 3, 5000, "Dieses Apartment ist bereits vermietet.");
                    return;
                }
                ...
                if (CharactersBank.GetBankAccountMoney(accNumber) < ServerHotels.GetApartmentPrice(hotelId, apartmentId)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Soviel Geld hast du auf deinem Konto nicht ({ServerHotels.GetApartmentPrice(hotelId, apartmentId)}$) - du hast {CharactersBank.GetBankAccountMoney(accNumber)}$");
                    return;
                }

                CharactersBank.SetBankAccountMoney(accNumber,
                    CharactersBank.GetBankAccountMoney(accNumber) - ServerHotels.GetApartmentPrice(hotelId, apartmentId));
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Ausgehende Überweisung", "Hotelzahlung",
                    $"Zimmerbuchung: {apartmentId}", $"+{ServerHotels.GetApartmentPrice(hotelId, apartmentId)}$", "Bankeinzug");
                ServerHotels.SetApartmentOwner(hotelId, apartmentId, charId);
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Sie haben sich das Zimmer mit der Zimmernummer '{apartmentId}' erfolgreich gemietet (Kosten: {ServerHotels.GetApartmentPrice(hotelId, apartmentId)}$). Dieses Zimmer läuft automatisch nach {ServerHotels.GetApartmentRentHours(hotelId, apartmentId)} Stunden ab.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public static void LockHotel(IPlayer player, int hotelId, int apartmentId) {
            try {
                ...
                if (!ServerHotels.HasCharacterAnApartment(charId)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Du besitzt hier kein Zimmer.");
                    return;
                }
        ...
        public void EnterHotel(IPlayer player, int hotelId, int apartmentId) {
            try {
                ...
                if (ServerHotels.GetApartmentOwner(hotelId, apartmentId) <= 0) {
                    HUDHandler.SendNotification(player, 3, 5000, "Fehler: Dieses Zimmer ist nicht vermietet.");
                    return;
                }
                ...
                if (hotel.isLocked) {
                    HUDHandler.SendNotification(player, 3, 5000, "Fehler: Das Zimmer ist abgeschlossen.");
                    return;
                }
        ...
        internal static void openStorage(IPlayer player) {
            try {
                ...
                var apartmentId = dimension - 5000;
                var hotelId = ServerHotels.GetHotelIdByApartmentId(apartmentId);
        ...
        public void StorageHotelItem(IPlayer player, int apartmentId, string itemName, int itemAmount, string fromContainer) {
            try {
                ...
                if (ServerHotels.GetHotelStorageItemWeight(apartmentId) >= 15f ||
                    ServerHotels.GetHotelStorageItemWeight(apartmentId) + itemWeight >= 15f) {
        ...
        public void TakeHotelItem(IPlayer player, int apartmentId, string itemName, int itemAmount) {
            try {
                ...
                if (!ServerHotels.ExistServerHotelStorageItem(apartmentId, itemName)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Der Gegenstand existiert im Lager nicht.");
                    return;
                }

                if (ServerHotels.GetServerHotelStorageItemAmount(apartmentId, itemName) < itemAmount) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Soviele Gegenstände sind nicht im Lager.");
                    return;
                }
                ...
                if (invWeight + itemWeight <= 15f) {
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast {itemName} ({itemAmount}x) aus deinem Lager genommen (Lagerort: Inventar).");
                ...
                if (Characters.GetCharacterBackpack(charId) != -2 && backpackWeight + itemWeight <=
                    ...
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast {itemName} ({itemAmount}x) aus deinem Lager genommen (Lagerort: Rucksack / Tasche).");
