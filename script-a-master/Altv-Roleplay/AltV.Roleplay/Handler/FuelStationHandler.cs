...
{
    internal class FuelStationHandler : IScript
    {
            ...
            try {
                ...
                if (ServerVehicles.GetVehicleType(vehicle) == 0) {
                    if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory")) {
                        HUDHandler.SendNotification(player, 3, 5000, "Du hast nicht genügend Bargeld dabei.");
                        return;
                    }

                    if (CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < selectedLiterPrice * selectedLiterAmount) {
                        HUDHandler.SendNotification(player, 3, 5000, "Du hast nicht genügend Bargeld dabei.");
                        return;
                    }
                }
                ...
                if (fuelStation == null) {
                    HUDHandler.SendNotification(player, 4, 5000, "Ein unerwarteter Fehler ist aufgetreten. [FEHLERCODE: FUEL-005]");
                    return;
                }

                var duration = 500 * selectedLiterAmount;
                ...
                var fuelVal = ServerVehicles.GetVehicleFuel(vehicle) + selectedLiterAmount;
                ...
                if (ServerVehicles.GetVehicleFuelTypeOnHash(vehicle.Model) != fueltype) {
                    ServerVehicles.SetVehicleEngineState(vehicle, false);
                    ServerVehicles.SetVehicleEngineHealthy(vehicle, false);
                    return;
                }

                ServerFuelStations.SetFuelStationBankMoney(fuelstationId,
                    ServerFuelStations.GetFuelStationBankMoney(fuelstationId) + selectedLiterPrice * selectedLiterAmount);

                if (ServerFuelStations.GetFuelStationOwnerId(fuelstationId) != 0)
                    ServerFuelStations.SetFuelStationAvailableLiters(fuelstationId,
                        ServerFuelStations.GetFuelStationAvailableLiters(fuelstationId) - selectedLiterAmount);
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
