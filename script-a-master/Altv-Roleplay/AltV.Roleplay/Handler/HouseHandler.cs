...
{
    internal class HouseHandler : IScript
    {
        internal static void openEntranceCEF(IPlayer player, int houseId) {
            try {
                ...
                player.EmitLocked("Client:HouseEntrance:openCEF", charId, ServerHouses.GetHouseInformationArray(houseId),
                    ServerHouses.IsCharacterRentedInHouse(charId, houseId));
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void BuyHouse(IPlayer player, int houseId) {
            try {
                ...
                if (ServerHouses.GetHouseOwner(houseId) > 0) {
                    HUDHandler.SendNotification(player, 3, 5000, "Fehler: Dieses Haus gehört bereits jemanden.");
                    return;
                }

                if (!CharactersBank.HasCharacterBankMainKonto(charId)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Du hast noch kein Hauptkonto in der Bank festgelegt.");
                    return;
                }
                ...
                if (CharactersBank.GetBankAccountLockStatus(accNumber)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Dein Bankkonto ist gesperrt.");
                    return;
                }
                ...
                if (CharactersBank.GetBankAccountMoney(accNumber) < ServerHouses.GetHousePrice(houseId)) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Soviel Geld hast du auf deinem Konto nicht ({ServerHouses.GetHousePrice(houseId)}$) - du hast {CharactersBank.GetBankAccountMoney(accNumber)}$");
                    return;
                }
                ...
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Ausgehende Überweisung", "Dynasty8",
                    $"Hauskauf: {ServerHouses.GetHouseStreet(houseId)}", $"-{ServerHouses.GetHousePrice(houseId)}$", "Bankeinzug");
                ServerHouses.SetHouseOwner(houseId, charId);
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Sie haben sich das Haus mit der Adresse '{ServerHouses.GetHouseStreet(houseId)}' erfolgreich gekauft (Kosten: {ServerHouses.GetHousePrice(houseId)}$).");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        internal static void LockHouse(IPlayer player, int houseId) {
            try {
                ...
                if (ServerHouses.GetHouseOwner(houseId) != charId && !ServerHouses.IsCharacterRentedInHouse(charId, houseId)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Dieses Haus gehört nicht dir und / oder du bist nicht eingemietet.");
                    return;
                }

                var lockState = ServerHouses.IsHouseLocked(houseId);
                ServerHouses.SetHouseLocked(houseId, !lockState);

                if (lockState)
                    HUDHandler.SendNotification(player, 2, 2500, "Du hast das Haus aufgeschlossen.");
                else
                    HUDHandler.SendNotification(player, 3, 2500, "Du hast das Haus abgeschlossen.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void EnterHouse(IPlayer player, int houseId) {
            try {
                ...
                if (ServerHouses.GetHouseOwner(houseId) <= 0) {
                    HUDHandler.SendNotification(player, 4, 5000, "Dieses Haus gehört Niemanden.");
                    return;
                }

                if (ServerHouses.IsHouseLocked(houseId)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Das Haus ist abgeschlossen.");
                    return;
                }

                var interiorId = ServerHouses.GetHouseInteriorId(houseId);
                if (interiorId <= 0) return;

                var targetPos = ServerHouses.GetInteriorExitPosition(interiorId);
        ...
        internal static void LeaveHouse(IPlayer player, int interiorId) {
            try {
                ...
                var housePos = ServerHouses.GetHouseEntrance(houseId);
        ...
        internal static void openStorage(IPlayer player) {
            try {
                ...
                if (!ServerHouses.HasHouseStorageUpgrade(houseId)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Dieses Haus besitzt noch keinen ausgebauten Lagerplatz.");
                    return;
                }
        ...
        public void StorageItem(IPlayer player, int houseId, string itemName, int itemAmount, string fromContainer) {
            try {
                ...
                var storageLimit = ServerHouses.GetInteriorStorageLimit(ServerHouses.GetHouseInteriorId(houseId));
                var itemWeight = ServerItems.GetItemWeight(itemName) * itemAmount;

                if (ServerHouses.GetHouseStorageItemWeight(houseId) >= storageLimit ||
                    ServerHouses.GetHouseStorageItemWeight(houseId) + itemWeight >= storageLimit) {
                    HUDHandler.SendNotification(player, 3, 5000,
                        $"Fehler: Soviel passt in das Hauslager nicht rein (maximal {storageLimit}kg Lagerplatz).");
                    return;
                }
        ...
        public void TakeItem(IPlayer player, int houseId, string itemName, int itemAmount) {
            try {
                ...
                if (!ServerHouses.ExistServerHouseStorageItem(houseId, itemName)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Der Gegenstand existiert im Hauslager nicht.");
                    return;
                }

                if (ServerHouses.GetServerHouseStorageItemAmount(houseId, itemName) < itemAmount) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Soviele Gegenstände sind nicht im Hauslager.");
                    return;
                }
                ...
                if (invWeight + itemWeight <= 15f) {
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast {itemName} ({itemAmount}x) aus dem Hauslager genommen (Lagerort: Inventar).");
                ...
                if (Characters.GetCharacterBackpack(charId) != -2 && backpackWeight + itemWeight <=
                    ...
                    HUDHandler.SendNotification(player, 2, 5000,
                        $"Du hast {itemName} ({itemAmount}x) aus dem Hauslager genommen (Lagerort: Rucksack / Tasche).");
        ...
        internal static async void BreakIntoHouse(IPlayer player, int houseId) {
            ...
            try {
                ...
                if (!CharactersInventory.ExistCharacterItem(charId, "Brecheisen", "inventory") &&
                    !CharactersInventory.ExistCharacterItem(charId, "Brecheisen", "backpack")) return;

                if (ServerFactions.GetFactionDutyMemberCount(2) < 6) {
                    HUDHandler.SendNotification(player, 3, 5000, "Es sind nicht genügend Beamte im Dienst (6).");
                    return;
                }

                if (!player.IsPlayerUsingCrowbar()) {
                    var houseOwner = ServerHouses.GetHouseOwner(houseId);
                    ...
                    if (DateTime.Now.Subtract(Convert.ToDateTime(Characters.GetCharacterLastLogin(houseOwner))).TotalHours >= 48) {
                        HUDHandler.SendNotification(player, 4, 5000, "Fehler: Der Hausbesitzer war in den letzten 48 Stunden nicht online.");
                        return;
                    }
                    ...
                    if (ServerHouses.HasHouseAlarmUpgrade(houseId)) {
                        ...
                        if (houseOwnerPlayer != null && (CharactersInventory.ExistCharacterItem(houseOwner, "Tablet", "inventory") ||
                            ...
                            HUDHandler.SendNotification(houseOwnerPlayer, 3, 3500,
                                $"Jemand bricht in dein Haus ein: {ServerHouses.GetHouseStreet(houseId)}");
                            HUDHandler.SendNotification(houseOwnerPlayer, 3, 3500,
                                $"Jemand bricht in dein Haus ein: {ServerHouses.GetHouseStreet(houseId)}");
                            HUDHandler.SendNotification(houseOwnerPlayer, 3, 3500,
                                $"Jemand bricht in dein Haus ein: {ServerHouses.GetHouseStreet(houseId)}");
                    ...
                    if (!player.Position.IsInRange(curPos, 3f)) {
                        HUDHandler.SendNotification(player, 3, 5000, "Aufbrechen abgebrochen, du bist zu weit entfernt.");
        ...
        public void setMainHouse(IPlayer player, int houseId) {
            try {
                ...
                if (!ServerHouses.ExistHouse(houseId) || ServerHouses.GetHouseOwner(houseId) != charId) {
                    HUDHandler.SendNotification(player, 4, 3500, "Fehler: Dieses Haus gehört nicht dir.");
                    return;
                }

                Characters.SetCharacterStreet(charId, ServerHouses.GetHouseStreet(houseId));
                HUDHandler.SendNotification(player, 2, 2500,
                    $"Du hast dich erfolgreich auf die Adresse '{ServerHouses.GetHouseStreet(houseId)} gemeldet.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void SellHouse(IPlayer player, int houseId) {
            try {
                ...
                if (!CharactersBank.HasCharacterBankMainKonto(charId)) {
                    HUDHandler.SendNotification(player, 3, 2500, "Du besitzt kein Haupt-Bankkonto");
                    return;
                }

                var accNumber = CharactersBank.GetCharacterBankMainKonto(charId);
                var housePrice = ServerHouses.GetHousePrice(houseId) / 2;
                ...
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Eingehende Überweisung", "Immobilienmakler",
                    $"Hausverkauf: {ServerHouses.GetHouseStreet(houseId)}", $"+{housePrice}$", "Online-Banking");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void DepositMoney(IPlayer player, int houseId, int money) {
            ...
            var dhouseId = dimension - 10000;
            if (dhouseId <= 0 || dhouseId != houseId || !ServerHouses.ExistHouse(houseId)) return;

            if (ServerHouses.GetHouseOwner(houseId) != charId) {
                HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du bist nicht der Hausbesitzer.");
                return;
            }

            if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "inventory") ||
                CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "inventory") < money) {
                HUDHandler.SendNotification(player, 4, 3500, $"Fehler: Du hast nicht genügend Geld dabei ({money}$).");
                return;
            }
            ...
            try {
                ...
                if (ServerHouses.GetHouseBankMoney(houseId) < money) {
                    HUDHandler.SendNotification(player, 4, 5000,
                        $"Fehler: Soviel Geld ist nicht im Tresor (Aktueller Stand: {ServerHouses.GetHouseBankMoney(houseId)}$).");
                    return;
                }
        ...
        public void BuyUpgrade(IPlayer player, int houseId, string upgrade) {
            try {
                ...
                switch (upgrade) {
                    case "alarm":
                        if (ServerHouses.HasHouseAlarmUpgrade(houseId)) {
                            HUDHandler.SendNotification(player, 4, 2500, "Dein Haus besitzt bereits eine Alarmanlage.");
                            return;
                        }

                        if (!ServerHouses.HasHouseBankUpgrade(houseId)) {
                            HUDHandler.SendNotification(player, 4, 2500, "Du hast noch keinen Tresor ausgebaut in dem genügend Geld ist (500$).");
                            return;
                        }

                        if (ServerHouses.GetHouseBankMoney(houseId) < 500) {
                            HUDHandler.SendNotification(player, 4, 2500, "Dein Haustresor verfügt nicht über die Kosten (500$).");
                            return;
                        }
                    ...
                    case "storage":
                        if (ServerHouses.HasHouseStorageUpgrade(houseId)) {
                            HUDHandler.SendNotification(player, 4, 2500, "Dein Haus besitzt bereits eine Lagermöglichkeit.");
                            return;
                        }

                        if (!ServerHouses.HasHouseBankUpgrade(houseId)) {
                            HUDHandler.SendNotification(player, 4, 2500, "Du hast noch keinen Tresor ausgebaut in dem genügend Geld ist (1500$).");
                            return;
                        }

                        if (ServerHouses.GetHouseBankMoney(houseId) < 1500) {
                            HUDHandler.SendNotification(player, 4, 2500, "Dein Haustresor verfügt nicht über die Kosten (1500$).");
                            return;
                        }
                    ...
                    case "bank":
                        if (ServerHouses.HasHouseBankUpgrade(houseId)) {
                            HUDHandler.SendNotification(player, 4, 2500, "Dein Haus besitzt bereits einen Tersor.");
                            return;
                        }

                        if (!CharactersBank.HasCharacterBankMainKonto(charId)) {
                            HUDHandler.SendNotification(player, 4, 5000, "Du besitzt noch kein Hauptkonto in deiner Bank.");
                            return;
                        }
                        ...
                        if (CharactersBank.GetBankAccountMoney(accNumber) < 250) {
                            HUDHandler.SendNotification(player, 4, 5000, "Dein Hauptkonto ist nicht ausreichend gedeckt (250$).");
                            return;
                        }
        ...
        public void RemoveRenter(IPlayer player, int houseId, int renterId) {
            try {
                ...
                if (!ServerHouses.IsCharacterRentedInAnyHouse(renterId) || !ServerHouses.IsCharacterRentedInHouse(renterId, houseId)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Dieser Spieler ist nicht in deinem Haus eingemietet.");
                    return;
                }
        ...
        public void setRentState(IPlayer player, int houseId, string rentState) {
            try {
                ...
                if (rentBool)
                    HUDHandler.SendNotification(player, 2, 2500, "Du hast den Mietstatus auf 'Mieter zulassen' gestellt.");
                else
                    HUDHandler.SendNotification(player, 2, 2500, "Du hast den Mietstatus auf 'Mieter nicht zulassen' gestellt.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void setRentPrice(IPlayer player, int houseId, int rentPrice) {
            try {
                ...
                if (rentPrice > 1000) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Die Miete darf einen Wert von 1.000$ nicht überschreiten.");
                    return;
                }
                ...
                HUDHandler.SendNotification(player, 2, 3000,
                    $"Du hast den Mietpreis auf {rentPrice}$ festgelegt. Die Miete wird jede 7 Tage von Mietbeginn eines Mieters abgebucht.");

                if (!ServerHouses.HasHouseBankUpgrade(houseId))
                    HUDHandler.SendNotification(player, 3, 5000,
                        "Dein Haus besitzt keinen Tresor. Mieteinnahmen werden erst gesammelt sofern dieser ausgebaut ist.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
        ...
        public void RentHouse(IPlayer player, int houseId) {
            try {
                ...
                if (ServerHouses.IsCharacterRentedInAnyHouse(charId)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du bist bereits in einem anderen Haus eingemietet.");
                    return;
                }

                if (ServerHouses.IsCharacterRentedInHouse(charId, houseId)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: In diesem Haus bist du bereits eingemietet.");
                    return;
                }

                if (!CharactersBank.HasCharacterBankMainKonto(charId)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Du besitzt kein Haupt-Bankkonto.");
                    return;
                }

                var accNumber = CharactersBank.GetCharacterBankMainKonto(charId);
                var ownerBankNumber = CharactersBank.GetCharacterBankMainKonto(ServerHouses.GetHouseOwner(houseId));
                var rentPrice = ServerHouses.GetHouseRentPrice(houseId);
                if (accNumber <= 0 || rentPrice <= 0 || ownerBankNumber <= 0) return;

                if (CharactersBank.GetBankAccountLockStatus(accNumber)) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Dein Hauptkonto ist gesperrt.");
                    return;
                }

                if (CharactersBank.GetBankAccountMoney(accNumber) < rentPrice) {
                    HUDHandler.SendNotification(player, 4, 5000, "Fehler: Dein Hauptkonto ist nicht ausreichend gedeckt.");
                    return;
                }
                ...
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Ausgehende Überweisung", $"{ownerBankNumber}",
                    "Mietvertrag", $"-{rentPrice}$", "Bankeinzug");
                ServerBankPapers.CreateNewBankPaper(ownerBankNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Eingehende Überweisung", $"{accNumber}", "Mietvertrag",
                    $"+{rentPrice}$", "Bankeinzug");
                HUDHandler.SendNotification(player, 2, 5000,
                    $"Du hast dich erfolgreich eingemietet, die Miete beträgt {rentPrice}$ täglich welche von deinem Hauptkonto abgezogen werden.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
