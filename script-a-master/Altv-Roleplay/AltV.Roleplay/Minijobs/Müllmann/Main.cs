    ...
    public class Main
    {
        ...
        public static void Initialize() {
            ...
            var data = new Server_Peds {
                model = "", posX = startJobShape.Position.X, posY = startJobShape.Position.Y, posZ = startJobShape.Position.Z,
                rotation = -0.9688125252723694f
            };
            ServerPeds.ServerPeds_.Add(data);
            var markerData = new Server_Markers {
                type = 33, posX = Constants.Positions.Minijob_Müllmann_VehOutPos.X, posY = Constants.Positions.Minijob_Müllmann_VehOutPos.Y,
                posZ = Constants.Positions.Minijob_Müllmann_VehOutPos.Z + 1, alpha = 150, bobUpAndDown = true, scaleX = 1, scaleY = 1, scaleZ = 1,
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat) {
            try {
                ...
                var rnd = new Random().Next(500, 850);
                ...
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Eingehende Überweisung", "Arbeitgeber: Müllmann",
            ...
            try {
                ...
                if (player.GetPlayerCurrentMinijobStep() == "FirstStepInVehicle") {
                    HUDHandler.SendNotification(player, 1, 2500,
                        "Fahre zum ersten Zielort um den Müll abzuholen, dieser wurde auf deinem GPS markiert.");
        ...
        private static void ColshapeEnterExitHandler(IColShape colShape, IEntity targetEntity, bool state) {
            try {
                ...
                if (client.GetPlayerCurrentMinijobStep() == "PICKUP_TRASH" && state && !client.IsInVehicle) {
                    ...
                    var personalThrowCol = Alt.GetAllColShapes().Where(x => x.Exists && x != null).ToList().FirstOrDefault(x =>
                        x != null && x.Exists && x.GetColShapeName() == "GarbageMinijobThrowInVehicle" && x.GetColShapeId() == charId);
                    ...
                    var MMThrowPos = veh.Position.getPositionInBackOfPosition(vehRot.Yaw, -5.5f);
                    client.EmitLocked("Client:Minijob:RemoveJobMarker");
                    client.SetPlayerCurrentMinijobStep("THROW_TRASH_IN_VEHICLE");
                    client.EmitLocked("Client:Minijob:CreateJobMarker", "Minijob: Müll einladen", 3, 514, 22, MMThrowPos.X, MMThrowPos.Y,
                        MMThrowPos.Z, true);
                    var throwCol = (ClassicColshape) Alt.CreateColShapeSphere(MMThrowPos, 2.5f);
                    throwCol.SetColShapeName("GarbageMinijobThrowInVehicle");
                    throwCol.SetColShapeId(charId);
                    ...
                    InventoryHandler.InventoryAnimation(client, "farmPickup", 1100);
                    var maxSpots = Model.GetMinijobGarbageMaxRouteSpots((int) client.GetPlayerCurrentMinijobRouteId());
                    ...
                    if ((int) client.GetPlayerCurrentMinijobActionCount() < maxSpots) {
                        ...
                        client.EmitLocked("Client:Minijob:CreateJobMarker", "Minijob: Müll abholen", 3, 514, 1, spot.posX, spot.posY, spot.posZ,
                            false);
                        HUDHandler.SendNotification(client, 1, 2500,
                            "Fahre zum nächsten Zielort um den Müll abzuholen, dieser wurde auf deinem GPS markiert.");
                        ...
                        if (personalThrowCol != null && personalThrowCol.Exists) personalThrowCol.Remove();
                        HUDHandler.SendNotification(client, 1, 6000,
                            "Alles aufgesammelt mein Jung. Zurück zur Mülldeponie - das Zeug abgeben, stell das Fahrzeug einfach dort ab wo du es bekommen hast.");
                        client.SetPlayerCurrentMinijobStep("DRIVE_BACK_TO_START");
                        client.EmitLocked("Client:Minijob:CreateJobMarker", "Minijob: Fahrzeug abgeben", 3, 514, 30,
                            Constants.Positions.Minijob_Müllmann_VehOutPos.X, Constants.Positions.Minijob_Müllmann_VehOutPos.Y,
                            Constants.Positions.Minijob_Müllmann_VehOutPos.Z, false);
                    }
        ...
        internal static void StartMinijob(IPlayer player) {
            try {
                if (player == null || !player.Exists) return;
                if (!startJobShape.IsInRange((ClassicPlayer) player)) return;
                ...
                if (player.GetPlayerCurrentMinijob() == "Müllmann") {
                    ...
                    var personalThrowCol = Alt.GetAllColShapes().Where(x => x.Exists && x != null).ToList().FirstOrDefault(x =>
                        x.GetColShapeName() == "GarbageMinijobThrowInVehicle" && x.GetColShapeId() == charId);
                    if (personalThrowCol != null && personalThrowCol.Exists) personalThrowCol.Remove();
                    ...
                    foreach (var veh in Alt.Server.GetVehicles().ToList()) {
                        ...
                        if (veh.Position.IsInRange(Constants.Positions.Minijob_Müllmann_VehOutPos, 5f)) {
                            HUDHandler.SendNotification(player, 3, 5000, "Der Ausparkpunkt ist belegt.");
                            return;
                        }
                    }
                    ...
                    HUDHandler.SendNotification(player, 1, 2500,
                        "Du hast den Minijob begonnen. Wir haben dir ein Fahrzeug zur Verfügung gestellt, steige in dies ein um zu beginnen.");
