    ...
    internal class Functions : IScript
    {
        ...
        public void GiveLicense(IPlayer player, int targetCharId, string licShort) {
            try {
                ...
                var accNumber = CharactersBank.GetCharacterBankMainKonto(targetCharId);
                var licPrice = CharactersLicenses.GetLicensePrice(licShort);
                ...
                ServerBankPapers.CreateNewBankPaper(accNumber, DateTime.Now.ToString("d", CultureInfo.CreateSpecificCulture("de-DE")),
                    DateTime.Now.ToString("t", CultureInfo.CreateSpecificCulture("de-DE")), "Ausgehende Überweisung", "Fahrschule",
                    $"Lizenzkauf: {CharactersLicenses.GetFullLicenseName(licShort)}", $"-{licPrice}$", "Bankeinzug");
                ...
                HUDHandler.SendNotification(player, 2, 2000,
                    $"Sie haben dem Spieler {Characters.GetCharacterName(targetCharId)} die Lizenz '{CharactersLicenses.GetFullLicenseName(licShort)}' für eine Gebühr i.H.v. {licPrice}$ ausgestellt.");
                HUDHandler.SendNotification(targetPlayer, 2, 2000,
                    $"Ihnen wurde die Lizenz '{CharactersLicenses.GetFullLicenseName(licShort)}' für eine Gebühr i.H.v. {licPrice}$ ausgestellt, diese wurde von Ihrem Hauptkonto abgebucht.");
            }
            catch (Exception e) {
                Alt.Log($"{e}");
            }
