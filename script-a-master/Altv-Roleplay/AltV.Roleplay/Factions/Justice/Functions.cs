        ...
        public void GiveWeaponLicense(IPlayer player, string targetCharName) {
            try {
                ...
                if (!Characters.ExistCharacterName(targetCharName)) {
                    HUDHandler.SendNotification(player, 3, 5000, $"Der angegebene Name wurde nicht gefunden ({targetCharName}).");
                    return;
                }
                ...
                if (!player.Position.IsInRange(targetPlayer.Position, 5f)) {
                    HUDHandler.SendNotification(player, 3, 5000, "Der Spieler ist nicht in Ihrer Nähe.");
                    return;
                }

                if (CharactersLicenses.HasCharacterLicense(targetCharId, "weaponlicense")) {
                    HUDHandler.SendNotification(player, 3, 5000, "Der Spieler hat bereits einen Waffenschein.");
                    return;
                }
        ...
        public void SearchBankAccounts(IPlayer player, string targetCharName) {
            try {
                ...
                if (targetBankAccounts == "[]" || targetBankAccounts == "") {
                    HUDHandler.SendNotification(player, 3, 5000, "Der Spieler hat keine Bankkonten.");
                    return;
                }
        ...
        public void ViewBankTransactions(IPlayer player, int accNumber) {
            try {
                ...
                if (!CharactersBank.ExistBankAccountNumber(accNumber)) {
                    HUDHandler.SendNotification(player, 3, 5000, $"Die ausgewählte Kontonummer existiert nicht ({accNumber}).");
                    return;
                }
                ...
                if (bankPapers == "[]" || bankPapers == "") {
                    HUDHandler.SendNotification(player, 3, 5000, "Dieses Konto besitzt keine Transaktionen.");
                    return;
                }
