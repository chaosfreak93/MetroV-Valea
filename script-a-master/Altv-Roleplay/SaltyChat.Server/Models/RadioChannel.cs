        ...
        internal void AddMember(VoiceClient voiceClient, bool isPrimary) {
            lock (_members) {
                ...
                foreach (var member in _members.Where(m => m.IsSending))
                    voiceClient.Player.EmitLocked("SaltyChat:PlayerIsSending", member.VoiceClient.Player, Name, true, false,
                        member.VoiceClient.Player.Position);
            }
        }
                ...
                if (member.IsSending) {
                    if (member.VoiceClient.IsRadioSpeakerEnabled)
                        foreach (var client in VoiceManager.Instance.VoiceClients)
                            client.Player.EmitLocked("SaltyChat:PlayerIsSendingRelayed", voiceClient.Player, Name, false, true,
                                voiceClient.Player.Position, false, Array.Empty<string>());
                    else
                        foreach (var client in VoiceManager.Instance.VoiceClients)
                            client.Player.EmitLocked("SaltyChat:PlayerIsSending", voiceClient.Player, Name, false, true,
                                voiceClient.Player.Position);
                }

                _members.Remove(member);

                foreach (var channelMember in _members.Where(m => m.IsSending))
                    voiceClient.Player.EmitLocked("SaltyChat:PlayerIsSending", channelMember.VoiceClient.Player, Name, false, false,
                        channelMember.VoiceClient.Player.Position);
        ...
        internal void SetSpeaker(VoiceClient voiceClient, bool isEnabled) {
            ...
            if (isEnabled || radioChannelMembers.Any(m => m.IsSpeakerEnabled))
                ...
                foreach (var client in VoiceManager.Instance.VoiceClients.Where(v => radioChannelMembers.All(m => m.VoiceClient != v)))
                    client.Player.EmitLocked("SaltyChat:PlayerIsSendingRelayed", sendingMember.VoiceClient.Player, Name, false, false,
                        sendingMember.VoiceClient.Player.Position, false, Array.Empty<string>());
        }
        ...
        internal void Send(VoiceClient voiceClient, bool isSending) {
            ...
            if (onSpeaker.Length > 0) {
                ...
                foreach (var remoteClient in VoiceManager.Instance.VoiceClients)
                    remoteClient.Player.EmitLocked("SaltyChat:PlayerIsSendingRelayed", voiceClient.Player, Name, isSending, stateChanged,
                        voiceClient.Player.Position, IsMember(remoteClient), channelMemberNames);
            } else {
                foreach (var member in channelMembers)
                    member.VoiceClient.Player.EmitLocked("SaltyChat:PlayerIsSending", voiceClient.Player, Name, isSending, stateChanged,
                        voiceClient.Player.Position);
            }
        }
