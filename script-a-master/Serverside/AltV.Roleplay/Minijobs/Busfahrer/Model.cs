...
namespace Altv_Roleplay.Minijobs.Busfahrer
{
    ...
    {
        public static List<Server_Minijob_Busdriver_Routes> ServerMinijobBusdriverRoutes_ = new();
        public static List<Server_Minijob_Busdriver_Spots> ServerMinijobBusdriverSpots_ = new();

        public static string GetAvailableRoutes(int charId) {
            if (charId <= 0) return "[]";

            var items = ServerMinijobBusdriverRoutes_.Where(x => CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer") >= x.neededExp)
                .Select(x => new {
                    x.id,
                    x.routeId,
                    x.routeName,
        ...
        public static Server_Minijob_Busdriver_Spots GetCharacterMinijobNextSpot(IPlayer player) {
            ...
            var spot = ServerMinijobBusdriverSpots_.FirstOrDefault(x =>
                x.routeId == (int) player.GetPlayerCurrentMinijobRouteId() && x.spotId == (int) player.GetPlayerCurrentMinijobActionCount());
