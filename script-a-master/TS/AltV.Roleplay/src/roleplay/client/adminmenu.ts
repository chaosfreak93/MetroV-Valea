...
alt.onServer("Client:HUD:CreateCEF", () => {
    ...
    adminMenuBrowser.on(
        "Client:AdminMenu:DoAction",
        (menuaction: string, info: string, addinfo: string, inputvalue: string) => {
...
alt.onServer("Client:AdminMenu:GetWaypointInfo", () => {
    ...
    if (native.doesBlipExist(waypoint)) {
        ...
        native.startPlayerTeleport(
            ...
            coords.y,
            coords.z + 1,
            alt.Player.local.rot.toDegrees().z,
            false,
            true,
            false,
        );
...
alt.onServer("Client:Adminmenu:TogglePlayerBlips", (info) => {
    if (info == "on") {
        for (let i = 0, n = alt.Player.all.length; i < n; i++) {
            ...
            playerblips_blip[player.scriptID] = BlipManager.createBlip(
                player.pos.x,
                player.pos.y,
                player.pos.z,
                0,
