...
import {
    getScaleformReturnValueIntAsync,
    getScaleformReturnValueBoolAsync,
    loadScaleformMovieAsync,
    registerTarget,
} from "../../utilities";
...
export default class SingleRace {
        ...
        if (native.isControlJustPressed(2, 237)) {
            ...
            if (ChooseHorseVisible) {
                if (clickedButton != 12 && clickedButton != -1) {
                    ...
                    alt.emitServer(
                        "Server:Casino:SingleRace:UpdateBetValues",
                        CurrentHorse,
                        CurrentBet,
                        CurrentGain,
                        true,
    ...
    static startRace(): void {
        ...
        native.playSoundFrontend(
            CurrentSoundId,
            "race_loop",
            "dlc_vw_casino_inside_track_betting_single_event_sounds",
            false,
        );
        SingleRace.GenerateHorsesOrder();
