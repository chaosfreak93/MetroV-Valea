...
let charcreatorBrowser: alt.WebView = null;
...
export default class CharCreator {
    static CreateCEF(player: alt.Player) {
        if (charcreatorBrowser == null) {
            ...
            charcreatorCam = native.createCameraWithParams(
            ...
            charcreatorBrowser.on("Client:Charcreator:ChangeGender", (gender: number) => {
                native.triggerScreenblurFadeOut(0);
            ...
            charcreatorBrowser.on("Client:Charcreator:SetRotation", (rot: string) => {
                native.setEntityHeading(pedHandle, parseFloat(rot));
            });

            charcreatorBrowser.on("Client:Charcreator:UpdateFaceFeature", (facefeaturesarray: string) => {
                const facefeaturesjson: JSON = JSON.parse(facefeaturesarray);
                for (let i = 0; i < 20; i++) {
                    native.setPedFaceFeature(pedHandle, i, parseFloat(facefeaturesjson[i]));
                }
            });

            charcreatorBrowser.on("Client:Charcreator:UpdateHeadBlends", (headblendsarray: string) => {
                const headblendsjson: JSON = JSON.parse(headblendsarray);
            ...
            charcreatorBrowser.on("Client:Charcreator:UpdateHeadOverlays", (headoverlaysarray: string) => {
                const headoverlaysjson: JSON = JSON.parse(headoverlaysarray);
            ...
            charcreatorBrowser.on(
                "Client:Charcreator:SaveCharacter",
                (
                    vorname: string,
                    nachname: string,
                    birthdate: Date,
                    gender: number,
                    facefeaturesarray: string,
                    headblendsdataarray: string,
                    headoverlaysarray: string,
                    clothesarray: string,
                ) => {
                    ...
                    alt.emitServer(
                        "Server:Charcreator:CreateCharacter",
                        vorname + " " + nachname,
                        birthdate,
                        gender,
                        facefeaturesarray,
    ...
    static showArea(area: string) {
        if (charcreatorBrowser != null) {
            charcreatorBrowser.emit("CEF:Charcreator:showArea", area);
        }
    }

    static async spawnCreatorPed(gender: number) {
        if (gender == 1) {
        ...
        native.setEntityHeading(pedHandle, 180.0);
