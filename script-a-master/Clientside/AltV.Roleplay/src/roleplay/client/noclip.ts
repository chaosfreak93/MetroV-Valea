...
export default class NoClip {
    ...
    static keyHandler(): void {
        ...
        if (native.isControlPressed(0, KEYS.FORWARD))
            currentPos = NoClip.addSpeedToVector(currentPos, dirForward, speed);
        if (native.isControlPressed(0, KEYS.BACKWARD))
            currentPos = NoClip.addSpeedToVector(currentPos, dirForward, -speed);
        if (native.isControlPressed(0, KEYS.LEFT))
            currentPos = NoClip.addSpeedToVector(currentPos, dirRight, -speed, true);
        if (native.isControlPressed(0, KEYS.RIGHT))
            currentPos = NoClip.addSpeedToVector(currentPos, dirRight, speed, true);
        ...
        if (!NoClip.isVectorEqual(new alt.Vector3(currentPos.x, currentPos.y, currentPos.z + zModifier), entity.pos))
