...
export default class Raycast {
    static line(radius: number, distance: number) {
    ...
    static result(ray: number) {
        let result = native.getShapeTestResult(ray, undefined, undefined, undefined, undefined);
        let hitEntity = result[4];
        if (
            !native.isEntityAPed(hitEntity) &&
            !native.isEntityAnObject(hitEntity) &&
            !native.isEntityAVehicle(hitEntity)
        )
            return undefined;
        return {
            isHit: result[1],
    ...
    static GetDirectionFromRotation(rotation: alt.Vector3): alt.Vector3 {
